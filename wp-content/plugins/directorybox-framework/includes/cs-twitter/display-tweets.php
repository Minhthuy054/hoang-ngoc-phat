...
if (!function_exists('get_auth')) {
	...
	$consumer_key = $wp_dp_cs_twitter_arg['consumerkey'];
	$consumer_secret = $wp_dp_cs_twitter_arg['consumersecret'];
	$user_token = $wp_dp_cs_twitter_arg['accesstoken'];
	$user_secret = $wp_dp_cs_twitter_arg['accesstokensecret'];
 ...
 */
if (!function_exists('cache_json')) {
    function cache_json( $id, $max_tweets, $time ) {
	
	require_once ( ABSPATH . '/wp-admin/includes/file.php' );
	
	$cache_dir = plugin_dir_path( __FILE__ ) . 'cache/';
	$cache = $cache_dir . $id . '.json'; // Twitter cache wp_dp_cs.
	$cache_folder = $cache_dir; // Twitter cache wp_dp_cs.
...
if (!function_exists('display_tweets')) {
	...
	if ( ! empty( $tweets ) ) {
		...
		foreach ( $tweets as $tweet ) {
			...
			if ( isset( $style ) ) {
				if ( ! empty( $style ) ) {
					$when = ( 'time_since' === $style ) ? '' : esc_html( wp_dp_cs_var_frame_text_srt( 'wp_dp_cs_var_tweets_time_on' ) );
					$twitter .= '<span>' . $when . '&nbsp;';
		...
		$twitter .= '<li>' . esc_html( wp_dp_cs_var_frame_text_srt( 'wp_dp_cs_var_no_tweets_found' ) ) . '</li>';
	} //end if statement.
