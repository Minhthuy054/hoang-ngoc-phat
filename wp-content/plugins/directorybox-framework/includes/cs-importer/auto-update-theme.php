...
add_action('admin_init', 'wp_dp_cs_check_for_updates');
if ( ! function_exists('wp_dp_cs_check_for_updates') ) {

	/**
	 * Check for theme updates if available
	 */
	function wp_dp_cs_check_for_updates() {
		global $wp_dp_cs_options;
		...
		if ( $upgrade_option ) {
			...
			if ( 'themes.php' == basename($_SERVER[PHP_SELF]) ) {
				add_action('admin_footer', 'wp_dp_cs_show_theme_auto_update_available_on_themes_page');
			}

			add_action('admin_notices', 'wp_dp_cs_show_theme_auto_update_available');
			return;
		}

		//- delete_option( 'last_time_auto_theme_update_checked_' . THEME_NAME );
		if ( ! empty($wp_dp_cs_options['cs_marketplace_username']) ) {
			...
			$user_name = $wp_dp_cs_options['cs_marketplace_username'];
			$api_key = $wp_dp_cs_options['cs_secret_api_key'];
			$skip_backup = $wp_dp_cs_options['cs_skip_theme_backup'];
			...
			if ( $errors = $protected_api->api_errors() ) {
				add_action('admin_notices', 'wp_dp_cs_show_theme_auto_update_error');
			} else {
			...
			add_action('admin_notices', 'wp_dp_cs_show_theme_auto_update_warning');
		}
	}
...
if ( ! function_exists('wp_dp_cs_show_theme_auto_update_available_on_themes_page') ) {

	/**
	 * Show theme update available on themes page, Ouput JS to footer of themes.php page.
	 */
	function wp_dp_cs_show_theme_auto_update_available_on_themes_page() {
...
if ( ! function_exists('wp_dp_cs_show_theme_auto_update_available') ) {

	/**
	 * Show notification if an update is avaiable.
	 */
	function wp_dp_cs_show_theme_auto_update_available() {
...
if ( ! function_exists('wp_dp_cs_show_theme_auto_update_error') ) {

	/**
	 * Show error if marketplace username or secret API keys is not valid.
	 */
	function wp_dp_cs_show_theme_auto_update_error() {
		$class = 'notice notice-error is-dismissible';
		//$message = __('Please provide valid Marketplace Username and Secret API Key in theme options to get Automatic Theme Updates.  &raquo; <a href="' . network_admin_url('admin.php?page=' . THEME_OPTIONS_PAGE_SLUG) . '#tab-auto-updater-show">Fix Now</a>', 'wp-dp-frame');
...
if ( ! function_exists('wp_dp_cs_show_theme_auto_update_warning') ) {

	/**
	 * Show Warning if auto updates are not configured.
	 */
	function wp_dp_cs_show_theme_auto_update_warning() {
		if ( false === ( $admin_dismiss_notice = get_transient('admin_dismiss_notice') ) ) {
			$class = 'notice notice-warning is-dismissible';
			//$message = __('Please configure Auto Updates in theme options to get Automatic Theme Updates. &raquo; <a href="' . network_admin_url('admin.php?page=' . THEME_OPTIONS_PAGE_SLUG) . '#tab-auto-updater-show">Configure Now</a>', 'wp-dp-frame');
...
if ( ! function_exists('wp_dp_cs_auto_upgrade_theme_and_plugins') ) {
	 ...
	 */
	function wp_dp_cs_auto_upgrade_theme_and_plugins() {
		global $wp_dp_cs_options, $current_screen, $cs_theme_upgraded_name, $cs_plugins_upgraded;
		...
		// Upgrade Theme.
		$theme_info['user_name'] = $wp_dp_cs_options['cs_marketplace_username'];
		$theme_info['api_key'] = $wp_dp_cs_options['cs_secret_api_key'];
		$theme_info['skip_backup'] = $wp_dp_cs_options['cs_skip_theme_backup'];
		$status = true;
		$status = wp_dp_cs_auto_update_theme($theme_info);

		// Upgrade Plugins.
		if ( function_exists('get_plugins_to_be_updated') ) {
			wp_dp_cs_auto_update_plugins(get_plugins_to_be_updated());
		}
...
if ( ! function_exists('wp_dp_cs_auto_update_theme') ) {
	 ...
	 */
	function wp_dp_cs_auto_update_theme($theme_info) {
		...
		$title = __('Update Theme', 'wp-dp-frame');
...
if ( ! function_exists('wp_dp_cs_auto_update_plugins') ) {
	 ...
	 */
	function wp_dp_cs_auto_update_plugins($plugins) {
		...
		foreach ( $plugins as $key => $plugin_info ) {
			if ( '' != $plugin_info['slug'] ) {
				...
				wp_dp_cs_include_file(ABSPATH . 'wp-admin/includes/class-wp-upgrader.php'); // Need for upgrade classes.

				$plugin = array();
				$plugin['slug'] = $plugin_info['slug']; // Needed for potentially renaming of wp_dp_cs name.
...
if ( ! function_exists('wp_dp_cs_backup_theme') ) {
	 ...
	 */
	function wp_dp_cs_backup_theme($theme) {
		$backup_errors = array();
