jQuery(document).ready(function () {
    jQuery('body').on('click', '#get-new-google-fonts', function () {
        ...
        var data = {
            action: 'wp_dp_google_fonts_refresh',
        };
        jQuery.post(ajaxurl, data, function (response) {
            ...
            if (count == 0) {
                ...
                var msg_res = msg + wp_dp_google_fonts.page_loading;
                window.location.reload(true);
            }
            jQuery('#wp-dp-fonts-msg').html('<div class="updated">' + msg_res + '</div>').hide();
            jQuery('#wp-dp-fonts-msg').slideDown(300);
            jQuery('#get-new-google-fonts').next('.spinner').css({'display': 'none'});
            setTimeout(function () {
                jQuery('#wp-dp-fonts-msg').slideUp(300);
            }, 4000);
        });
    });
    ...
    jQuery('body').on('click', '.add-google-font', function () {
        ...
        if (!button.hasClass('font-added'))
        {
            ...
            var data = {
                action: 'wp_dp_add_selected_google_font',
            ...
            jQuery.post(ajaxurl, data, function (response) {
                ...
                button.val(wp_dp_google_fonts.added_in);
                ...
                if (variants_array.length > 0 || subsets_array.length > 0)
                {
                    ...
                    font_html += '<div class="font-select-holder">';
                        font_html += '<span class="select-butns">';
                            font_html += '<a class="select-all" date-font_slug="'+ font_slug +'">'+ wp_dp_google_fonts.select_all +'</a>';
                            font_html += '<a class="unselect-all" date-font_slug="'+ font_slug +'">'+ wp_dp_google_fonts.unselect_all +'</a>';
                        font_html += '</span>';
                    font_html += '</div>';
    ...
    jQuery('body').on('click', '.font-delete', function () {
        ...
        jQuery('body').append('<div class="google-font-confirmation"><div class="google-font-confirmation-header"><h3>' + wp_dp_google_fonts.are_you_sure_remove_font + '</h3></div><div class="google-font-message"><input type="button" id="" data-google-font_name="' + font_name + '" class="google-font-message-buttons google-font-message-delete google-font-buttons" value="' + wp_dp_google_fonts.yes + '"/><input type="button" id="" class="google-font-message-buttons google-font-message-no-delete google-font-buttons" value="' + wp_dp_google_fonts.no + '"/></div><a href="javacript:void(0)" class="google-font-anchor-buttons google-font-buttons google-font-message-no-delete"><i class="dashicons dashicons-no-alt"></i></a></div>');
    ...
    jQuery('body').on('click', '.google-font-buttons', function () {
        if (jQuery(this).hasClass('google-font-message-delete')) {
            ...
            var data = {
                action: 'wp_dp_delete_google_font',
                font_name: font_name
            };
            jQuery.post(ajaxurl, data, function (response) {
    ...
    jQuery('body').on('click', '.update-google-font-button', function () {
        ...
        var data = {
            action: 'wp_dp_update_google_font',
            font_name: font_name,
...
function wp_dp_get_google_fonts() {
    ...
    jQuery.post(ajaxurl, data, function (response) {
        ...
        {
            var ghtml = '<div class="google-font">'+ wp_dp_google_fonts.seems_dont_have_font +' <a href="javascript:void(0);" id="get-new-google-fonts">'+ wp_dp_google_fonts.font_just_click +'.</a> <span class="spinner"></span></div>';
        ...
        } else
        {
            if (fonts.length == 0)
            {
                ...
                ghtml += wp_dp_google_fonts.not_font_search;
                ghtml += '</div>';
                $google_fonts_list_wrapper.html(ghtml);
            } else
            {
...
function convert_json_to_html(object) {
    ...
    jQuery.each(object, function (index, google_font) {
        ...
        if (selected == 'true') {
            var button_text = wp_dp_google_fonts.added_in;
            var button_class = 'font-added';
        } else {
            var button_text = wp_dp_google_fonts.add_to;
            var button_class = '';
        }
