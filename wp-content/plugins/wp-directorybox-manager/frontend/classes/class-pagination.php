...
if ( ! class_exists( 'Wp_dp_Pagination' ) ) {

	Class Wp_dp_Pagination {

		public function __construct() {
			add_action( 'wp_dp_pagination', array( $this, 'wp_dp_pagination_callback' ), 11, 1 );
		}

		public function wp_dp_pagination_callback( $args ) {
			$total_posts = '';
			$posts_per_page = '5';
			$paging_var = 'paged_id';
			$show_pagination = 'yes';
			...
			if ( $show_pagination <> 'yes' ) {
				...
				if ( $total_posts > $posts_per_page ) {
					if ( $paged_id > 1 ) {
						$html .= '<li><a href="?' . $paging_var . '=' . ($paged_id - 1) . $qrystr . '"  class="prev page-numbers">' . wp_dp_plugin_text_srt( 'wp_dp_pagination_prev' ) . ' </a></li>';
					} else {
				...
				if ( $total_posts > $posts_per_page ) {
					if ( $total_posts > 0 and $paged_id < ($total_posts / $posts_per_page) ) {
						$html .= '<li><a class="next page-numbers" href="?' . $paging_var . '=' . ($paged_id + 1) . $qrystr . '" >' . wp_dp_plugin_text_srt( 'wp_dp_pagination_next' ) . '</a></li>';
					} else {
	...
	global $Wp_dp_Pagination;
	$Wp_dp_Pagination = new Wp_dp_Pagination();
}
?>