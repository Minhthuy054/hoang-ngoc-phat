 ...
 */
if(!class_exists('Wp_dp_Email_Templates_Functions')){
class Wp_dp_Email_Templates_Functions {
    ...
    public function __construct() {
        ...
        add_action('wp_dp_email_template', array($this, 'jh_email_notification'), 10, 1);
    ...
    public function plugin_enqueue() {
        ...
        wp_enqueue_style('wp-dp-email-templates', WP_DP_EMAIL_TEMPLATES_PLUGIN_URL . '/assets/css/wp-dp-email-templates.css');
        wp_enqueue_script('wp-dp-email-templates', WP_DP_EMAIL_TEMPLATES_PLUGIN_URL . '/assets/js/wp-dp-email-templates.js', array('jquery'));
    }
    ...
    public function jh_email_notification($atts = array()) {
        ...
        if (!empty($job_id)) {
            ...
            $jh_user = get_post_meta($job_id, 'wp_dp_job_username', true);
        ...
        $headers = Wp_dp_Email_Templates_Functions::email_header($email_from, $email_reply_to);
    ...
    public static function send_email_with_template_callback($email_template_index, $template_type) {
        ...
        if (isset($wp_dp_plugin_options[$email_template_index])) {
            $selected_template_id = intval($wp_dp_plugin_options[$email_template_index]);
            ...
            if ($selected_template_id != 0) {
                ...
                if (isset(Wp_dp_Email_Templates_Post::$email_template_options)) {
                    if (isset(Wp_dp_Email_Templates_Post::$email_template_options['templates'][$template_type])) {
                        // Get default template.
                        $email_template = Wp_dp_Email_Templates_Post::$email_template_options['templates'][$template_type];
                    }
                }
            }
            // Get Email template types in Wp_dp_Email_Templates_Post class.
            if (isset(Wp_dp_Email_Templates_Post::$email_template_options)) {
                $email_template_variables = array_merge(
                        Wp_dp_Email_Templates_Post::$email_template_options['variables']['General'], Wp_dp_Email_Templates_Post::$email_template_options['variables'][$template_type]
                );
            }
        }
        ...
        if ($email_template == null) {
            $email_template = wp_dp_plugin_text_srt('wp_dp_template_fun');
        }
    }
...
$wp_dp_email_functions = new Wp_dp_Email_Templates_Functions();
}