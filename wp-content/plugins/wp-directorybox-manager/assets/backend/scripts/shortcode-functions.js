 ...
 *Shortcode
 */
function wp_dp_shortocde_selection(val, admin_url, id) {
    "use strict";
    SuccessLoader();
    jQuery("#" + id).parents('#cs-widgets-list').removeClass('wide-width');
    if (val != "" && val != "Shortcode") {
	jQuery('#cs-shrtcode-loader').html('<i class="fancy-spinner"></i>');
...
function SuccessLoader() {
    ...
    var loader = "<div class='loader'><i class='fancy-spinner'></i></div>"
 ...
 */
function wp_dp_page_composer_filterable(id) {
    "use strict";
...
jQuery(document).on("change", ".get-listing-cats-by-type", function () {
    var listing_type   = jQuery(this).val();
    var rand_id = jQuery(this).data('id');
    $.ajax({
        type: "POST",
        url: wp_dp_globals.ajax_url,
        data: 'listing_type='+listing_type+'&listing_element_rand_id='+rand_id+'&action=get_listing_cats_by_type',
