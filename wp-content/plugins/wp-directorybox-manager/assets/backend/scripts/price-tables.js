$(document).ready(function ($) {
	...
	wp_dp_pt_package_fixed_fields();

	//append the new column
	$("#wp-dp-price-table-" + _this_id).find('tr.pt_row').each(function () {
	    var rand_id = Math.floor((Math.random() * 99999999) + 1);
	    $(this).append('<td>' + pt_col_input + '<br><span id="icon-' + rand_id + '"></span></td>');

	    //icon loader
	    wp_dp_pt_icon(rand_id);
	});

	//appending row_num_input
	$("#wp-dp-price-table-" + _this_id + " tr.pt_row td:first").find('input.row_num_input').remove();
	$("#wp-dp-price-table-" + _this_id + " tr.pt_row td:first").append(row_num_input);

	//setting section col span
	var dir_num_cols = $("#wp-dp-price-table-" + _this_id + " tr.default-fields:first td").length;
	$("#wp-dp-price-table-" + _this_id + " tr.pt_section td:nth-child(2)").attr('colspan', (dir_num_cols - 1));

	//appending actions
	wp_dp_action_cols(_this_id, 'column');
	...
	if (dir_num_cols > 1) {
	    ...
	    $("#wp-dp-price-table-" + _this_id + " tbody").append("\n<tr class='pt_row'>\n" + wp_dp_num_of_cols(dir_num_cols - 1) + "\n\n</tr>\n");

	    //appending actions
	    wp_dp_action_cols(_this_id, 'row');

	    //adding title field
	    $("#wp-dp-price-table-" + _this_id + " tbody tr:last td:first").append('<br><input type="text" name="dir_pt_row_title[]" value="Row Title">');

	    //icon loader
	    $("#wp-dp-price-table-" + _this_id + " tbody tr:last span[id^=\"icon-\"]").each(function () {
	...
	if (dir_num_cols > 1) {
	    ...
	    var pt_sec_pos_val = $("#wp-dp-price-table-" + _this_id + " .row_num_input").length;
	    ...
	    //appending actions
	    wp_dp_action_cols(_this_id, 'section');
	} else {
	    alert('Add a Package first.')
	}
	...
	var _this_id = $(this).data('id');
	alert($("#wp-dp-price-table-" + _this_id)[0].outerHTML);
    ...
    function wp_dp_num_of_cols(dir_num_cols) {//begin function
        "use strict";
    ...
    function wp_dp_action_cols(id, part) {//begin function
        "use strict";
	var table = $("#wp-dp-price-table-" + id);
	...
	//number of td elements in the first row
	var cols = [];
	var dir_num_cols = $("#wp-dp-price-table-" + id + " tr.default-fields:first td").length;
	var pt_col_joins;
	//loop through the dir_num_cols
	for (var i = 0; i < dir_num_cols; i++) {//begin for loop
	...
	if (_table.find('input').length == 0) {
	    _table.html(wp_dp_pt_default_html());
	}
    });
	...
	var default_data = wp_dp_pt_default_html();

	var r = confirm("Are you sure! This will remove all data.");
	if (r == true) {
	    $("#wp-dp-price-table-" + _this_id).html(default_data);
	} else {
	    return false;
	}
    ...
    function wp_dp_pt_icon(id) {//begin function
        "use strict";
	var getting_icon;
    ...
    function wp_dp_pt_package_fixed_fields() {//begin function
        "use strict";
	$('tr.default-fields').each(function () {
		...
		$(this).append('<td class="pkg-url">' + wp_dp_pt_vars.packages_dropdown + '</td>');
	    }
	});

    }//end function

    function wp_dp_pt_default_html() {
        "use strict";
	var default_data = '\
        <tbody>\
