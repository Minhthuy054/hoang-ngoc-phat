...
jQuery(document).on("click", "#add_member", function () {
    ...
    wp_dp_show_loader("#add_member", "", "button_loader", thisObj);
    var serializedValues = jQuery("#team_add_form").serialize();
    var form = $('#team_add_form')[0];
    var form_data = new FormData(form);
    form_data.append('action', 'wp_dp_add_team_member');
    jQuery.ajax({
        type: "POST",
        dataType: "json",
        contentType: false,
        ...
        success: function (response) {
            wp_dp_show_response(response, ".team-fields", thisObj);
        }
    });
});
...
jQuery(document).on("click", "#team_update_form", function () {
    ...
    var thisObj = jQuery(this);
    wp_dp_show_loader(thisClass, "", "button_loader", thisObj);
    var serializedValues = jQuery("#wp_dp_update_team_member" + user_id).serialize();
    var form = $("#wp_dp_update_team_member" + user_id)[0];
    var form_data = new FormData(form);
    form_data.append('wp_dp_user_id', user_id);
    form_data.append('action', 'wp_dp_update_team_member');
    jQuery.ajax({
        type: "POST",
        dataType: "json",
        contentType: false,
...
function wp_dp_member_content(counter, char, page_var, post_per_page, page_num) {

    "use strict";

    wp_dp_show_loader(".member-alphabatic");
    ...
    jQuery.ajax({
        ...
        dataType: "HTML",
        url: wp_dp_globals.ajax_url,
        data: "page_num=" + page_num + "&post_per_page=" + post_per_page + "&counter=" + counter + "&page_var=" + page_var + "&char=" + char + "&action=wp_dp_member_content",
        success: function (response) {
...
function wp_dp_member_pagenation_ajax(page_var, page_num, counter, post_per_page) {
    "use strict";
    jQuery("#" + page_var + "-" + counter).val(page_num);
    wp_dp_member_content(counter, "", page_var, post_per_page, page_num);
}

jQuery(document).on("click", "#profile_form", function () {
    "use strict";
    var returnType = wp_dp_validation_process(jQuery("#member_profile"));
    if (returnType == false) {
        return false;
    }
    var thisObj = jQuery(this);
    wp_dp_show_loader("#profile_form", "", "button_loader", thisObj);
    ...
    if (jQuery('#wp_dp_member_profile_image').length) {
        var files = jQuery('#wp_dp_member_profile_image').prop('files');
        if (files.length > 0) {
            jQuery.each(jQuery('#wp_dp_member_profile_image'), function (i, obj) {
                jQuery.each(obj.files, function (j, file) {
                    data.append('wp_dp_member_profile_image', file);
                })
            });
        }
    }
    var other_data = jQuery("#member_profile").serializeArray();
    ...
    jQuery.ajax({
        ...
        contentType: false,
        success: function (response) {
            if (jQuery("#member_profile").find("#wp_dp_member_display_name").length > 0) {
                var display_name = jQuery("#member_profile #wp_dp_member_display_name").val();
                wp_dps_update_display_name(display_name);
            }
            ...
            if (jQuery("#cropContainerModal").find("img.croppedImg2").length > 0) {
                ...
                wp_dps_appned_profile_image(upload_image_src, default_image_url, true);
            } else if (jQuery("#cropContainerModal").find("img").length > 0) {
                var image_src = jQuery("#cropContainerModal img").attr("src");
                wp_dps_appned_profile_image(image_src, default_image_url, false);
            }
            wp_dp_show_response(response, "", thisObj);
        }
    });
...
function wp_dps_update_display_name(display_name) {
    "use strict";
...
function wp_dps_appned_profile_image(image_url, default_image_url, appned_span) {
    "use strict";
    if (image_url != "") {
...
jQuery(document).on("click", ".ad-submit", function () {
    "use strict";
    var thisObj = jQuery(this);
    wp_dp_show_loader(".ad-submit", "", "button_loader", thisObj);
});
...
jQuery(document).on("click", "#member_change_password", function () {
    "use strict";
    var returnType = wp_dp_validation_process(jQuery("#change_password_form"));
    if (returnType == false) {
        return false;
    }
    var thisObj = jQuery("#member_change_password");
    wp_dp_show_loader("#member_change_password", "", "button_loader", thisObj);
...
jQuery(document).on("click", "#member_change_address", function () {
    ...
    // Get all the forms elements and their values in one ste
    var thisObj = jQuery(this);
    wp_dp_show_loader("#member_change_address", "", "button_loader", thisObj);
...
jQuery(document).on("click", "#member-book-day-off-btn", function () {
    "use strict";
    var thisObj = jQuery(this);
    wp_dp_show_loader("#member-book-day-off-btn", "", "button_loader", thisObj);
    var serializedValues = jQuery("#member-book-day-off-form").serialize();
    jQuery.ajax({
        type: "POST",
        dataType: "json",
        url: wp_dp_globals.ajax_url,
        data: serializedValues + "&action=wp_dp_member_book_day_off_submission",
        success: function (response) {
            wp_dp_show_response(response, "", thisObj);
        }
    });
...
jQuery(document).on("click", ".uploaded-img li img", function () {
    ...
    jQuery("#cropContainerModal figure a img").attr("src", img_src);
    jQuery("#wp_dp_saved_member_profile_image").val(attachment_id);
    jQuery("#cropContainerModal").attr("data-img-type", "selective");
});


jQuery(document).on("click", ".upload-file .upload-img", function () {
    "use strict";
    $(".user-profile-images").find("input[type='file']").trigger('click');
});

jQuery(document).on("change", "#wp_dp_member_profile_image", function (event) {
    var image_size = this.files[0].size / 1024 / 1024;
    if (image_size <= 1) {
       var tmppath = URL.createObjectURL(event.target.files[0]);
       jQuery("#cropContainerModal figure a img").attr("src", tmppath);
    }else {
...
function wp_dp_user_permission($this, add_member_permission, condition) {
    "use strict";
...
jQuery(document).on("click", ".remove_branche", function () {
    ...
    jQuery.ajax({
        ...
        dataType: "json",
        url: wp_dp_globals.ajax_url,
        data: "wp_dp_branch_id=" + branch_id + "&action=wp_dp_remove_member_branch",
        success: function (response) {
            if (response.type == "success") {
                jQuery("#wp_dp_member_change_locations").trigger("click");
            }
            wp_dp_show_response(response);
        }
    });
...
jQuery(document).on("click", ".branch-option .add_branch", function (e) {
    ...
    e.preventDefault();
    var thisObj = jQuery(this);
    wp_dp_show_loader(".add_branch", "", "button_loader", thisObj);
    jQuery.ajax({
        type: "POST",
        dataType: "html",
        url: wp_dp_globals.ajax_url,
        data: "action=wp_dp_member_branch_add",
        success: function (response) {
...
jQuery(document).on("click", ".member-branch-det", function (e) {
    ...
    jQuery.ajax({
        ...
        dataType: "html",
        url: wp_dp_globals.ajax_url,
        data: "wp_dp_branch_id=" + branch_id + "&action=wp_dp_member_branch_add",
        success: function (response) {
            jQuery("#add_update_branch").html(response);
            jQuery(".add-update-branch").addClass("active");
            jQuery("body").append('<div id="overlay" style="display:none"></div>');
...
jQuery(document).on("click", "#save_branch", function () {
    "use strict";
    var thisObj = jQuery(this);
    wp_dp_show_loader("#save_branch", "", "button_loader", thisObj);
    var serializedValues = jQuery("#branch_add_form").serialize();
    jQuery.ajax({
        type: "POST",
        dataType: "json",
        url: wp_dp_globals.ajax_url,
        data: serializedValues + "&action=wp_dp_add_update_branch",
        success: function (response) {
            wp_dp_show_response(response, ".branch-fields", thisObj);
            setTimeout(function () {
                if (response.type == "success") {
                    jQuery(".add-update-branch").removeClass("active");
                    jQuery("#overlay").fadeOut(300);
...
jQuery(document).on("click", ".member-thumbnail-upload", function () {
    var data_id = jQuery(this).data('id');
    jQuery("#wp_dp_member_thumb_" + data_id).click();
});

jQuery(document).on("change", ".wp-dp-member-thumb", function (event) {
    var data_id = jQuery(this).data('id');
    var tmppath = URL.createObjectURL(event.target.files[0]);
...
jQuery(document).on("click", ".remove-member-thumb", function () {
    ...
    jQuery("#wp_dp_member_thumb_id_" + data_id).val('');
});