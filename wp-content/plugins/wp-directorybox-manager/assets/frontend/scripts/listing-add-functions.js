...
$(document).on('submit', 'form.wp-dp-dev-payment-form', function () {
    ...
    wp_dp_show_loader('.payment-submit-btn', '', 'button_loader', thisObj);
});
...
$(document).on('submit', 'form.wp-dp-dev-listing-form', function () {
    ...
    var thisObj = jQuery(".add-listing-loader");
    wp_dp_show_loader('.add-listing-loader', '', 'button_loader', thisObj);
});
...
function validate_register_add_restaurant_form(that) {
    ...
    if ($("input[name='wp_dp_listing_package']").length > 0) {
        if ($("input[name='wp_dp_listing_package']:checked").length < 1) {
            form_validity = 'invalid';
            $("input[name='wp_dp_listing_package']").closest('.wp-dp-pkg-header').css({"border": "1px solid #ff0000"});
        } else {
            $("input[name='wp_dp_listing_package']").closest('.wp-dp-pkg-header').css({"border": "1px solid #f4f4f4"});
        }
    }
...
$(document).on('change', '.wp-dp-dev-username, .wp-dp-dev-user-email', function () {
    ...
    var checkig_user,
            ...
            _this_val = $(this).val(),
            ajax_url = $("#wp-dp-dev-posting-main-" + _this_id).data('ajax-url'),
            _plugin_url = $("#wp-dp-dev-posting-main-" + _this_id).data('plugin-url'),
            color,
            this_loader;
...
$(document).on('click', '.wp-dp-dev-select-type', function () {
    "use strict";
    var selecting_type,
            _this_id = $(this).data('id'),
            _this_val = $(this).val(),
            _this_form = $("#wp-dp-dev-listing-form-" + _this_id),
            ajax_url = $("#wp-dp-dev-posting-main-" + _this_id).data('ajax-url'),
            _plugin_url = $("#wp-dp-dev-posting-main-" + _this_id).data('plugin-url'),
            cf_append = $('#wp-dp-dev-cf-con-' + _this_id),
            main_append = $('#wp-dp-dev-main-con-' + _this_id),
            after_append = $('#wp-dp-dev-main-con-' + _this_id).find('#wp-dp-type-sec-' + _this_id),
            this_loader = $('#wp-dp-dev-loader-' + _this_id);
    
    this_loader.html('<div class="loader-holder"><img src="' + _plugin_url + 'assets/frontend/images/ajax-loader.gif" alt=""></div>');
    selecting_type = $.ajax({
        url: ajax_url,
        method: "POST",
        data: 'select_type=' + _this_val + '&listing_add_counter=' + _this_id + '&action=wp_dp_listing_load_cf' + '&' + _this_form.serialize(),
        dataType: "json"
    }).done(function (response) {
        ...
        if (typeof response.main_html !== 'undefined') {
            main_append.find('li.wp-dp-dev-appended').remove();
    ...
    if (window.File && window.FileList && window.FileReader) {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            if (!file.type.match('image')) {
                alert(wp_dp_listing_strings.upload_images_only);
                image_file = false;
            }
...
$(document).on('click', '.wp-dp-dev-cancel-pkg', function () {
    ...
    $('.all-pckgs-sec').find('.wp-dp-pkg-header').removeAttr('style');
...
$(document).on('click', '.pkg-cancel-btn', function () {
    ...
    package_detail_sec.slideUp(400, function () {
        _this.parents('.wp-dp-pkg-holder').find('.wp-dp-pkg-header').removeAttr('style');
    });
...
$(document).on('click', '.pkg-choose-btn', function () {
    ...
    package_detail_sec.slideUp(400, function () {
        _this.parents('.wp-dp-pkg-holder').find('.wp-dp-pkg-header').css({'background-color': '#b7b7b7'});
    });
});
...
$(document).on('click', 'a[id^="wp-dp-dev-insert-opening_house-"]', function () {
    var _this_id = $(this).data('id'),
            _this_con = $('#wp-dp-dev-insert-opening_house-con-' + _this_id);
    _this_con.slideToggle();
...
$(document).on('click', 'a[id^="wp-dp-dev-add-opening_house-"]', function () {
    "use strict";
    ...
    var     _this_ = $(this),
	    ...
	    _time_to_field = _this_con.find('.opening-house-time-to'),
            _this_append = $('#wp-dp-dev-add-opening_house-app-' + _this_id),
    ...
    if (typeof _date_field !== 'undefined' && _date_field.val() != '') {
        ...
        var thisObj = jQuery('#wp-dp-dev-add-opening_house-'+ _this_id +'');
        wp_dp_show_loader('#wp-dp-dev-add-opening_house-'+ _this_id +'', '', 'button_loader', thisObj);
	...
	data.append('time_to_field', _time_to_field.val());
        data.append('listing_add_counter', _this_id);
        data.append('action', 'wp_dp_listing_opening_house_to_list');
        ...
        var adding_opening_house = $.ajax({
            ...
            if (typeof response.html !== 'undefined') {
                ...
                _this_append.append(response.html);
                _date_field.val('');
                $('#wp-dp-dev-insert-opening_house-con-' + _this_id).find('.icons-selector .selector .selected-icon > i').removeAttr('class');
                $('#wp-dp-dev-insert-opening_house-con-' + _this_id).find('.icons-selector .selector .selected-icon > i').addClass('icon-cancel2');
                _time_from_field.val('');
		_time_to_field.val('');
	    }
            var response = {
                type: 'success',
                msg: wp_dp_listing_strings.opening_house_added
            };
            wp_dp_show_response(response, '', thisObj);
            $('#wp-dp-dev-insert-opening_house-con-' + _this_id).slideUp();
        }).fail(function () {
            wp_dp_show_response('', '', thisObj);
        });
    } else {
...
$(document).on('click', 'a[id^="wp-dp-dev-opening_house-edit-"]', function () {
    var _this_id = $(this).data('id'),
            _this_con = $('#wp-dp-dev-opening_house-edit-con-' + _this_id);
    _this_con.slideToggle();

});

$(document).on('click', 'a[id^="wp-dp-dev-opening_house-save-"]', function () {
    "use strict";
    var _this_id = $(this).data('id'),
    _this_con = $('#wp-dp-dev-opening_house-edit-con-' + _this_id),
    ...
    var thisObj = jQuery('#wp-dp-dev-opening_house-save-'+ _this_id +'');
    wp_dp_show_loader('#wp-dp-dev-opening_house-save-'+ _this_id +'', '', 'button_loader', thisObj);
