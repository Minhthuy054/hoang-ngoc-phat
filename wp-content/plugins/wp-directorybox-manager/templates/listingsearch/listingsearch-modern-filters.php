	...
	<div class="search-default-fields <?php echo esc_html( $listing_search_fieled_class ); ?>">
		<?php if ( $listingsearch_title_switch == 'yes' ) { ?>
			<div class="field-holder search-input">
				<strong class="search-title" <?php echo wp_dp_allow_special_char( $label_style_colr ); ?>><?php echo wp_dp_plugin_text_srt( 'wp_dp_listing_search_view_enter_kywrd_label' ); ?></strong>
				<label>
		...
		if ( $listingsearch_location_switch == 'yes' ) {
			?>
			<div class="field-holder search-input">
				<strong class="search-title" <?php echo wp_dp_allow_special_char( $label_style_colr ); ?>><?php echo wp_dp_plugin_text_srt( 'wp_dp_listing_search_view_enter_location_label' ); ?></strong>
				<?php
				$wp_dp_select_display = 1;
				wp_dp_get_custom_locations_listing_filter( '<div id="wp-dp-medern-location-select-holder" class="search-country" style="display:' . wp_dp_allow_special_char( $wp_dp_select_display ) . '"><div class="select-holder">', '</div></div>', false, $listing_short_counter );
				?>
			</div>
			<?php
		}
		if ( $listingsearch_listing_type_switch == 'yes' ) {
			...
			<div id="listing_type_select_fields_<?php echo wp_dp_allow_special_char( $listing_short_counter ); ?>" class="listing-type-cate-fields field-holder select-dropdown has-icon">
				<strong class="search-title" <?php echo wp_dp_allow_special_char( $label_style_colr ); ?>><?php echo wp_dp_plugin_text_srt( 'wp_dp_listing_search_view_enter_type_label' ); ?></strong>
				<label>
					<?php
					...
					$wp_dp_opt_array = array(
						...
						'options' => $types_array,
						'extra_atr' => ' onchange="wp_dp_listing_type_search_fields(this,\'' . $listing_short_counter . '\',\'' . $listingsearch_price_switch . '\'); wp_dp_listing_type_cate_fields(this,\'' . $listing_short_counter . '\',\'' . $listingsearch_categories_switch . '\',\'modern\',\'' . $wp_dp_search_label_color . '\'); "',
					);
		...
		if ( $listingsearch_advance_filter_switch == 'yes' ) {
			...
			<div class="field-holder advanced-btn">
				<a href="javascript:void(0);" onclick="wp_dp_advanced_search_field('<?php echo wp_dp_allow_special_char( $listing_short_counter ); ?>');" <?php echo wp_dp_allow_special_char( $label_style_colr ); ?>><?php echo wp_dp_plugin_text_srt( 'wp_dp_listing_search_flter_advanced' ); ?><i class="icon-chevron-small-down"></i></a>
			</div>
		<?php } ?>
