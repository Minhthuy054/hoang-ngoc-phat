    ...
    <div class="search-default-fields <?php echo esc_html( $listing_search_fieled_class ); ?>">
		...
		if ( $listingsearch_location_switch == 'yes' ) {
			...
			<div class="field-holder search-input">
				...
				wp_dp_get_custom_locations_listing_filter( '<div id="wp-dp-list-location-select-holder" class="search-country" style="display:' . wp_dp_allow_special_char( $wp_dp_select_display ) . '"><div class="select-holder">', '</div></div>', false, $listing_short_counter );
				?>
			</div>
			<?php
		}
		if ( $listingsearch_listing_type_switch == 'yes' ) {
			?>
			<div class="field-holder select-dropdown listing-type"> 
				<?php
				$wp_dp_post_listing_types = new Wp_dp_Post_Listing_Types();
				$all_type_label = wp_dp_plugin_text_srt( 'wp_dp_listing_listing_type_alll' );
				$listing_types_array = $wp_dp_post_listing_types->wp_dp_types_array_callback( $all_type_label );
				...
				$wp_dp_opt_array = array(
					'std' => $listing_typ,
					'cust_id' => 'search_form_listing_type',
			...
			<div id="listing_type_cate_fields_<?php echo wp_dp_allow_special_char( $listing_short_counter ); ?>" class="listing-category-fields field-holder select-dropdown has-icon">
				<label>
                    ...
                    <script type="text/javascript">
                            jQuery(".listing-category-fields #wp_dp_listing_category").chosen({disable_search: true});
                    </script>
			<?php
		}
		if ( $listingsearch_advance_filter_switch == 'yes' ) {
			...
			<div class="field-holder advanced-btn">
				<a href="javascript:void(0);" onclick="wp_dp_advanced_search_field('<?php echo wp_dp_allow_special_char( $listing_short_counter ); ?>');"><i class="icon-cog"></i><?php echo wp_dp_plugin_text_srt( 'wp_dp_listing_search_flter_advanced' ); ?></a>
			</div>
		<?php } ?>
