...
if ( isset($_POST['payment_status']) && $_POST['payment_status'] == 'Completed' &&
	...
	if ( isset($booking_id) && ! empty($booking_id) ) {
		$wp_dp_pack_tra_meta = get_post_meta($booking_id, "dir_pakage_transaction_meta", true);
		if ( is_int($wp_dp_pack_tra_meta) ) {
			$wp_dp_pack_tra_meta = array();
		}
		if ( $wp_dp_pack_tra_meta == '' ) {
			$wp_dp_pack_tra_meta = array();
		}
		if ( ! is_array($wp_dp_pack_tra_meta) || empty($wp_dp_pack_tra_meta) || $wp_dp_pack_tra_meta == '' ) {
			$wp_dp_pack_tra_meta = array();
		}
		$trans_counter = 0;
		if ( is_array($wp_dp_pack_tra_meta) && count($wp_dp_pack_tra_meta) > 0 ) {
			$trans_counter = count($wp_dp_pack_tra_meta);
		}
		...
		$index_count = 0;
		$wp_dp_tra_meta = get_option('wp_dp_booking_transaction_meta', true);
		if ( is_int($wp_dp_tra_meta) ) {
			$wp_dp_tra_meta = array();
		}

		if ( ! isset($wp_dp_tra_meta) || empty($wp_dp_tra_meta) || ! is_array($wp_dp_tra_meta) ) {
			$wp_dp_tra_meta = array();
		}

		if ( isset($wp_dp_tra_meta[$booking_id]) && is_array($wp_dp_tra_meta[$booking_id]) && count($wp_dp_tra_meta[$booking_id]) > 0 ) {
			$index_count = (int) count($wp_dp_tra_meta[$booking_id]);
		}
		 ...
		 * All Transactions Data Saved
		  /*---------------------------------------------------- */
		$wp_dp_booking_status = isset($wp_dp_plugin_options['wp_dp_booking_visibility']) ? $wp_dp_plugin_options['wp_dp_booking_visibility'] : 'pending';
		$package_featured_ads = isset($wp_dp_plugin_options['booking_featured_ad_price']) ? $wp_dp_plugin_options['booking_featured_ad_price'] : 0;

		$wp_dp_tra_meta[$booking_id][$index_count][$tnx_type] = $_POST;
		update_option('wp_dp_booking_transaction_meta', $wp_dp_tra_meta);
		$booking_post = array();
		...
		if ( isset($_POST['txn_id']) && $_POST['txn_id'] <> '' ) {
			...
			$wp_dp_pack_tra_meta[$trans_counter] = $transaction_array;

			$payment_date = date_i18n('Y/m/d H:i:s', strtotime(esc_attr($_POST['payment_date'])));

			update_post_meta((int) $booking_id, 'dir_pakage_transaction_meta', $wp_dp_pack_tra_meta);
			...
			$postStatus['post_status'] = $wp_dp_booking_status;
			wp_update_post($postStatus);

			/* ----------------------------------------------------	
			 * Update Featured Status
			  /*---------------------------------------------------- */
			update_post_meta($booking_id, 'wp_dp_booking_pkg_names', $package_id);

			if ( $package_id == '0000000000' ) {
				$package_meta = get_post_meta($booking_id, "_pakage_meta", true);
			} else {
				$wp_dp_packages_options = get_option('wp_dp_packages_options');
				$package_meta = $wp_dp_packages_options[$package_id];
			}

			if ( isset($package_meta['package_duration']) && $package_meta['package_duration'] == 'unlimited' ) {

				update_post_meta((int) $booking_id, 'dir_pkg_expire_date', $wp_dp_current_date);
			 ...
			 * Update Package Add Till Date
			  /*---------------------------------------------------- */
			if ( isset($featured) && $featured == 'yes' ) {
				$featured_days = isset($wp_dp_plugin_options['booking_featured_ad_days']) ? $wp_dp_plugin_options['booking_featured_ad_days'] : 0;
