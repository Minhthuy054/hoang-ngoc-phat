    ...
    class Payment_Processing {
        ...
        public function custom_order_status_cancelled($order_id) {
            ...
            if ( isset($rcv_parameters) && ! empty($rcv_parameters) ) {
                ...
                $redirect_url = add_query_arg($_REQUEST, $wp_dp_plugin_options['wp_dp_dir_paypal_ipn_url']);
        ...
        public function custom_thankyou_page($order_id) {
            ...
            if ( isset($rcv_parameters) && ! empty($rcv_parameters) ) {
                ...
                $order_status_array = array(
                    ...
                    'order_id' => $order_id,
                    'status_code' => 200,
                    'status_message' => wp_dp_plugin_text_srt('wp_dp_wooc_hooks_order_received'),
                );
                $return_url = get_option('wooCommerce_current_page');
                if ( ! isset($return_url) || $return_url == '' ) {
                    $return_url = site_url();
        ...
        public function action_woocommerce_new_order($order_id) {
            ...
            $transaction_id = $rcv_parameters['custom_var']['wp_dp_transaction_id'];
            if ( isset($rcv_parameters) && ! empty($rcv_parameters) ) {
            ...
            update_post_meta($transaction_id, 'wp_dp_transaction_pay_method', get_post_meta($order_id, '_payment_method', true));
            update_post_meta($transaction_id, 'wp_dp_currency', get_woocommerce_currency_symbol());
            update_post_meta($transaction_id, 'wp_dp_currency_position', wp_dp_get_currency_position());
        ...
        public function custom_payment_complete($order_id) {
            ...
            $_REQUEST['payment_source'] = 'WP_DP_WOOCOMMERCE_GATEWAY';
            $redirect_url = $wp_dp_plugin_options['wp_dp_dir_paypal_ipn_url'];
        ...
        public function woocommerce_cart_calculate_fees_callback($wooccm_custom_user_charge_man) {
            global $woocommerce, $wp_dp_plugin_options;

            $vat_tax = 0;
            if ( isset($wp_dp_plugin_options['wp_dp_vat_switch']) && $wp_dp_plugin_options['wp_dp_vat_switch'] == 'on' ) {
                $vat_tax = ( isset($wp_dp_plugin_options['wp_dp_payment_vat']) && $wp_dp_plugin_options['wp_dp_payment_vat'] != '' ) ? $wp_dp_plugin_options['wp_dp_payment_vat'] : 0;
            }
            if ( $vat_tax != 0 ) {
                $items = $woocommerce->cart->get_cart();

                foreach ( $items as $item ) {
                    $product = isset($item['data']->post) ? $item['data']->post : array();
                    $product_id = isset($product->ID) ? $product->ID : 0;
                }
                $rcv_parameters = get_post_meta($product_id, 'rcv_parameters', true);
				$transaction_id = isset( $rcv_parameters['custom_var']['wp_dp_transaction_id'] ) ? $rcv_parameters['custom_var']['wp_dp_transaction_id'] : '';
                $wp_dp_transaction_amount = isset($rcv_parameters['price']) ? $rcv_parameters['price'] : 0;

                $wp_dp_vat_amount = $wp_dp_transaction_amount * ( $vat_tax / 100 );
                $vat_amount = WP_DP_FUNCTIONS()->num_format($wp_dp_vat_amount);
				
				update_post_meta( $transaction_id, 'wp_dp_transaction_origional_price', $wp_dp_transaction_amount );
				update_post_meta( $transaction_id, 'wp_dp_transaction_vat_price', $vat_amount );
				update_post_meta( $transaction_id, 'wp_dp_transaction_vat_tax', $vat_tax );

                $woocommerce->cart->add_fee(wp_dp_plugin_text_srt('wp_dp_wooc_hooks_vat') . ' (' . $vat_tax . '%)', $vat_amount);
            }
