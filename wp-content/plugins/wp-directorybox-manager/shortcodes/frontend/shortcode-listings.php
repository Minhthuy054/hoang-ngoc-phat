 ...
 */
if (!class_exists('Wp_dp_Shortcode_Listings_Frontend')) {

    class Wp_dp_Shortcode_Listings_Frontend {

        /**
         * Constant variables
         */
        var $PREFIX = 'wp_dp_listings';
        ...
        public function __construct() {
            ...
            add_action('wp_ajax_wp_dp_listings_content', array($this, 'wp_dp_listings_content'));
            add_action('wp_ajax_nopriv_wp_dp_listings_content', array($this, 'wp_dp_listings_content'));
            ...
            add_action('wp_dp_draw_search_element', array($this, 'wp_dp_draw_search_element_callback'), 11, 1);
        }
        ...
        public function wp_dp_listings_shortcode_callback($atts, $content = "") {
            ...
            if (is_user_logged_in()) {
                wp_enqueue_script('wp-dp-hide-listing-functions');
            }
        ...
        public function wp_dp_listings_content($listing_arg = '') {
            ...
            if (isset($_REQUEST['listing_arg']) && $_REQUEST['listing_arg']) {
                $listing_arg = stripslashes(html_entity_decode($_REQUEST['listing_arg']));
                $listing_arg = json_decode($listing_arg);
            ...
            $element_listing_recent_switch = isset($atts['listing_recent_switch']) ? $atts['listing_recent_switch'] : 'no';
            $element_listing_recent_count = isset($atts['listing_recent_count']) ? $atts['listing_recent_count'] : '5';
            ...
            $listing_right_sidebar_content = isset($content) ? $content : '';
            ...
            $listing_desc = isset($atts['listing_desc']) ? $atts['listing_desc'] : '';
            $listing_cus_fields = isset($atts['listing_cus_fields']) ? $atts['listing_cus_fields'] : '';
            ...
            //
            // If featured listing.
            if ($listing_listing_urgent == 'only-urgent') {
                $element_filter_arr[] = array(
            ...
            $page_id = isset($_REQUEST['page_id']) ? $_REQUEST['page_id'] : get_the_ID();
            ...
            } elseif ($listing_sort_by == 'low_price') {
                $qryvar_listing_sort_type = 'ASC';
                $qryvar_sort_by_column = 'meta_value_num';
                $meta_key = 'wp_dp_listing_price';
            } elseif ($listing_sort_by == 'high_price') {
                $qryvar_listing_sort_type = 'DESC';
                $qryvar_sort_by_column = 'meta_value_num';
                $meta_key = 'wp_dp_listing_price';
            ...
            // query for recent listing start
            $recent_listing_args = array();
            if ($element_listing_recent_switch == 'yes') {
                $element_recent_listing_filter_arr[] = array(
                    'key' => 'wp_dp_listing_posted',
                    'value' => strtotime(current_time($default_date_time_formate, 1)),
                    'compare' => '<=',
                );
                ...
                $recent_listing_args = array(
                    'posts_per_page' => $element_listing_recent_count,
                    'post_type' => 'listings',
                    'post_status' => 'publish',
                    'order' => 'DESC',
                    'orderby' => 'post_date',
                    'fields' => 'ids', // only load ids
                    'meta_query' => array(
                        $element_recent_listing_filter_arr,
                    ),
                );
            }
            $all_post_ids = isset( $all_post_ids )? $all_post_ids : array();
            if (isset($_REQUEST['loc_polygon_path']) && $_REQUEST['loc_polygon_path'] != '') {
                $loc_polygon_path = $_REQUEST['loc_polygon_path'];
                $all_post_ids = $this->listing_polygon_filter($loc_polygon_path, $all_post_ids, $element_filter_arr);
            }
            ...
            <form id="frm_listing_arg<?php echo absint($listing_short_counter); ?>">
                ...
                <?php
                if ($search_box_sidebar == 'yes' && isset($listing_view) && $listing_view != 'map' && $split_map == true && $wp_dp_map_position != 'top') {  // if sidebar on from element
                ...
                echo '<div class="row">';
                if (( $search_box_sidebar == 'yes' && isset($listing_view) && $listing_view != 'map' && $split_map == false) || (!empty($wp_dp_listing_sidebar) ) || ($split_map == true && $wp_dp_map_position == 'top')) {  // if sidebar on from element
                    set_query_var('listing_type', $listing_type);
                    ...
                    set_query_var('listing_view', $listing_view);
                    set_query_var('args_count', $args_count);
                    set_query_var('listing_right_sidebar_content', $listing_right_sidebar_content);
                    set_query_var('atts', $atts);
                    set_query_var('listing_totnum', $listing_totnum);
                    set_query_var('page_url', $page_url);
                    set_query_var('listing_loop_obj', $listing_loop_obj);
                ...
                } else if (isset($listing_right_sidebar_content) && $listing_right_sidebar_content != '') {
                    $content_columns = 'col-lg-9 col-md-9 col-sm-12 col-xs-12';
                }
                ?> 
                <div class="<?php echo esc_html($content_columns); ?>">
                    ...
                    ?>
                    <div class="directorybox-listing-content directorybox-dev-listing-content12" id="wp-dp-data-listing-content-<?php echo esc_html($listing_short_counter); ?>" data-id="<?php echo esc_html($listing_short_counter); ?>">
                        <?php
                        $listings_title = isset($atts['listings_title']) ? $atts['listings_title'] : '';
                        ...
                        $listing_element_seperator = isset($atts['wp_dp_listings_seperator_style']) ? $atts['wp_dp_listings_seperator_style'] : '';


                        $wp_dp_listings_element_title_color = isset($atts['wp_dp_listings_element_title_color']) ? $atts['wp_dp_listings_element_title_color'] : '';
                        $wp_dp_listings_element_subtitle_color = isset($atts['wp_dp_listings_element_subtitle_color']) ? $atts['wp_dp_listings_element_subtitle_color'] : '';
                        $element_title_color = '';
                        if (isset($wp_dp_listings_element_title_color) && $wp_dp_listings_element_title_color != '') {
                            $element_title_color = ' style="color:' . $wp_dp_listings_element_title_color . ' ! important"';
                        }
                        $element_subtitle_color = '';
                        if (isset($wp_dp_listings_element_subtitle_color) && $wp_dp_listings_element_subtitle_color != '') {
                            $element_subtitle_color = ' style="color:' . $wp_dp_listings_element_subtitle_color . ' ! important"';
                        }


                        if ($listings_title != '' || $listings_subtitle != '') {
                            ?>
                            <div class="row">
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                    <div class="element-title <?php echo ($listings_title_alignment); ?>">
                                        <?php
                                        if ($listings_title != '' || $listings_subtitle != '') {
                                            if ($listings_title != '') {
                                                ?>
                                                <h2<?php echo wp_dp_allow_special_char($element_title_color); ?>><?php echo esc_html($listings_title); ?></h2>
                                                <?php
                                            }
                                            if ($listings_subtitle != '') {
                                                ?>
                                                <p <?php echo wp_dp_allow_special_char($element_subtitle_color); ?>><?php echo esc_html($listings_subtitle); ?></p>
                                                <?php
                                            }
                                            if (isset($listing_element_seperator) && !empty($listing_element_seperator)) {
                                                $wp_dp_listing_seperator_html = '';
                                                if ($listing_element_seperator == 'classic') {
                                                    $wp_dp_listing_seperator_html .='<div class="classic-separator ' . $listings_title_alignment . '"><span></span></div>';
                                                }
                                                if ($listing_element_seperator == 'zigzag') {
                                                    $wp_dp_listing_seperator_html .= '
													<div class="separator-zigzag ' . $listings_title_alignment . '">
														<figure><img src="' . trailingslashit(wp_dp::plugin_url()) . 'assets/images/zigzag-img1.png" alt=""/></figure>
													</div>';
                                                }
                                                echo force_balance_tags($wp_dp_listing_seperator_html);
                                            }
                                        }
                                        ?>
                                    </div>
                        ...
                        if ((isset($listing_view) && $listing_view != 'map')) {
                            ...
                            echo '<div class="split-map-fixed-filter">';
                            $this->listing_search_sort_fields($atts, $listing_sort_by, $listing_short_counter, $listing_view, $listing_totnum);
                            echo '</div>';
                            echo '</div>';
                        }
                        ...
                        set_query_var('listing_loop_obj', $listing_loop_obj);
                        set_query_var('listing_view', $listing_view);
                        set_query_var('listing_desc', $listing_desc);
                        set_query_var('listing_cus_fields', $listing_cus_fields);
                        set_query_var('listing_short_counter', $listing_short_counter);
                        set_query_var('atts', $atts);
                        ...
                        set_query_var('listing_hide_switch', $listing_hide_switch);
                        if ($element_listing_top_category == 'yes') {
                ...
                <?php
                if (isset($listing_right_sidebar_content) && !empty($listing_right_sidebar_content)) {
                    echo '<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12 sticky-map-sidebar">';
                    echo do_shortcode($listing_right_sidebar_content);
                    echo '</div>';
                }
                echo '</div>';
        ...
        public function listing_polygon_filter($polygon_pathstr, $post_ids, $custom_meta_array = '') {
            global $wpdb;
            if (empty($post_ids)) {
                if (isset($custom_meta_array) && !empty($custom_meta_array) && is_array($custom_meta_array)) {
                    $post_ids = wp_dp_get_query_whereclase_by_array($custom_meta_array);
                }
            }
            $polygon_path = array();
            ...
            foreach ($post_ids as $listing_id) {
                $qry = "SELECT meta_value FROM $wpdb->postmeta WHERE 1=1 AND post_id='" . $listing_id . "' AND meta_key='wp_dp_post_loc_latitude_listing'";
                $listing_latitude_arr = $wpdb->get_col($qry);
                $listing_latitude = isset($listing_latitude_arr[0]) ? $listing_latitude_arr[0] : '';

                $qry = "SELECT meta_value FROM $wpdb->postmeta WHERE 1=1 AND post_id='" . $listing_id . "' AND meta_key='wp_dp_post_loc_longitude_listing'";
                $listing_longitude_arr = $wpdb->get_col($qry);
                $listing_longitude = isset($listing_longitude_arr[0]) ? $listing_longitude_arr[0] : '';
        ...
        public function listing_search_features_filter() {
        ...
        public function listing_search_sort_fields($atts, $listing_sort_by, $listing_short_counter, $view = '', $listing_totnum = '') {
            ...
            if (( isset($atts['listing_sort_by']) && $atts['listing_sort_by'] != 'no') || ( isset($atts['listing_layout_switcher']) && $atts['listing_layout_switcher'] != 'no' ) || $view_type == true) {
                ?>
                        ...
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"> 
                            <div class="all-results">
                                <div class="split-map-heading">
                                    <h5>
                                        <?php
                                        if (($view == 'list' || $view == 'grid' ) && $view_type != 'split_map') {
                                            echo absint($listing_totnum) . '&nbsp;';
                                            if ($listing_totnum == 1) {
                                                echo wp_dp_plugin_text_srt('wp_dp_list_view_listing_found');
                                            } else {
                                                echo wp_dp_plugin_text_srt('wp_dp_list_view_listings_found');
                                            }
                                        } else {
                                            echo absint($listing_totnum) . '&nbsp;';
                ...
                $args_more = array(
                    ...
                    'listing_price_filter' => isset($atts['listing_price_filter']) && $atts['listing_price_filter'] != '' ? ( $atts['listing_price_filter'] ) : '',
                            ...
                            foreach ($element_listing_layout_switcher_view as $single_layout_view) {
                                ...
                                switch ($case_for_list) {
                                    ...
                                    case 'grid-default':
                                        ...
                                        $icon .= wp_dp_plugin_text_srt('wp_dp_element_view_gid_default');
        ...
        public function listing_location_filter($location_slug, $all_post_ids) {
            ...
            if (($search_type == 'autocomplete' && ($radius == 0 || $radius == '') ) || ($search_type == 'custom' && ($radius == 0 || $radius == '') )) {
                if (isset($location_slug) && $location_slug != '') {
                    $term_exit = term_exists($location_slug, 'wp_dp_locations');
                    if ($term_exit !== 0 && $term_exit !== null) {
                        $term = get_term_by('name', $location_slug, 'wp_dp_locations');
        ...
        public function listing_price_filter($open_house = '', $all_post_ids = '') {
            ...
            if ($show_pagination <> 'yes') {
                ...
                $html .= $wp_dp_form_fields_frontend->wp_dp_form_hidden_render(
                        array(
                            ...
                            'std' => '',
                            'extra_atr' => 'onchange="wp_dp_listing_content(\'' . $listing_short_counter . '\', \'' . $view_type . '\');"',
                        )
                );
                $html .= '<div class="page-nation"><ul class="pagination pagination-large">';
                if ($paged_id > 1) {
                    $html .= '<li class="pagination-prev"><a onclick="wp_dp_listing_pagenation_ajax(\'' . $paging_var . '\', \'' . ($paged_id - 1) . '\', \'' . ($listing_short_counter) . '\', \'' . ($ajax_filter) . '\', \'' . ($view_type) . '\');" href="javascript:void(0);">';
                ...
                if ($paged_id >= 3 and $total_page > 5) {
                    ...
                    $html .= '<li><a onclick="wp_dp_listing_pagenation_ajax(\'' . $paging_var . '\', \'' . (1) . '\', \'' . ($listing_short_counter) . '\', \'' . ($ajax_filter) . '\', \'' . ($view_type) . '\');" href="javascript:void(0);">';
                    $html .= '1</a></li>';
                }
                if ($paged_id >= 3 and $total_page > 5) {
                ...
                if ($total_page > 1) {
                    ...
                    for ($i = $loop_start; $i <= $loop_end; $i ++) {
                        ...
                        if ($i <> $paged_id) {
                            ...
                            $html .= '<li><a onclick="wp_dp_listing_pagenation_ajax(\'' . $paging_var . '\', \'' . ($i) . '\', \'' . ($listing_short_counter) . '\', \'' . ($ajax_filter) . '\', \'' . ($view_type) . '\');" href="javascript:void(0);">';
                            $html .= $i . '</a></li>';
                        } else {
                ...
                if ($loop_end <> $total_page) {
                    $html .= '<li><a onclick="wp_dp_listing_pagenation_ajax(\'' . $paging_var . '\', \'' . ($total_page) . '\', \'' . ($listing_short_counter) . '\', \'' . ($ajax_filter) . '\', \'' . ($view_type) . '\');" href="javascript:void(0);">';
                    $html .= $total_page . '</a></li>';
                }
                if ($total_posts > 0 and $paged_id < ($total_posts / $posts_per_page)) {
                    $html .= '<li class="pagination-next"><a onclick="wp_dp_listing_pagenation_ajax(\'' . $paging_var . '\', \'' . ($paged_id + 1) . '\', \'' . ($listing_short_counter) . '\', \'' . ($ajax_filter) . '\', \'' . ($view_type) . '\');" href="javascript:void(0);">';
        ...
        public function wp_dp_listing_categories_list($listing_type, $parent_cate_string) {
            ...
            if ($parent_cate_string != '') {
                $category_request_val_arr = is_array( $parent_cate_string )? $parent_cate_string : explode(",", $parent_cate_string);
        ...
        public function directorybox_duplicate_post_as_draft() {
            set_time_limit(0);
            global $wpdb;
            if (!( isset($_REQUEST['post']) && ( isset($_REQUEST['action']) && 'directorybox_duplicate_post_as_draft' == $_REQUEST['action'] ) )) {
                wp_die('No post to duplicate has been supplied!');
            }
            echo 'directorybox_duplicate_post_as_draft 3';
            $count = 1;
            /*
        ...
        public function wp_dp_all_listings_by_s($s, $record = '-1') {
            ...
            $args = array(
                ...
                'post_type' => array('listings', 'listing_type'),
                'post_status' => 'publish',
                's' => $s,
                'fields' => 'ids', // only load ids
                'meta_query' => array(
            ...
            $listing_loop_obj = wp_dp_get_cached_obj('listing_autocomplete_result_cached_loop_obj', $args, 12, false, 'wp_query');
    ...
    $wp_dp_shortcode_listings_frontend = new Wp_dp_Shortcode_Listings_Frontend();
}

if (!function_exists('listing_search_keywords')) {

    function listing_search_keywords($listing_totnum, $element_listing_search_keyword, $qrystr = '', $atts = array(), $page_url = '') {
        if ($element_listing_search_keyword != 'no' || ( isset($atts['listing_alert_button']) && $atts['listing_alert_button'] == 'enable')) {
            ?>
            <div class="search-options">
                <div class="all-results">
                    <?php if ($element_listing_search_keyword != 'no') { ?>
                        <h6><?php
                            $reset_var = 0;
                            if (isset($qrystr)) {
                                $qry_flag = 0;
                                foreach ($qrystr as $qry_var => $qry_val) {
                                    if ('ajax_filter' == $qry_var || 'advanced_search' == $qry_var || 'listing_arg' == $qry_var || 'action' == $qry_var || 'alert-frequency' == $qry_var || 'alerts-name' == $qry_var || 'loc_polygon' == $qry_var || 'alerts-email' == $qry_var || 'loc_polygon_path' == $qry_var)
                                        continue;
                                    $qry_flag ++;
                                    if ($qry_val != '') {
                                        $reset_var ++;
                            ...
                            echo wp_dp_plugin_text_srt('wp_dp_listings_search_criteria');
                            ?></h6>
                        <?php if (isset($reset_var) && $reset_var > 0) { ?>
                            <a class="clear-tags reset-results" href="<?php echo esc_url($page_url); ?>"><i class=" icon-cross3"></i><?php wp_dp_plugin_text_srt('wp_dp_listings_reset'); ?></a>
                            <?php
                        }
                        ?>
