 ...
 *  set query vars
 */
if ( ! function_exists('wp_dp_query_vars') ) {

    function wp_dp_query_vars($vars) {
        $vars[] = 'social-login';
        return $vars;
    }

    add_action('query_vars', 'wp_dp_query_vars');
}
/*
 *  set parse request
 */
if ( ! function_exists('wp_dp_parse_request') ) {

    function wp_dp_parse_request($wp) {
    ...
    add_action('parse_request', 'wp_dp_parse_request');
}

/*
 *  login process method
 */
if ( ! function_exists('wp_dp_social_process_login') ) {

    function wp_dp_social_process_login($is_ajax = false) {
        global $wp_dp_plugin_options, $wpdb, $buyer_permissions;
        ...
        $wp_dp_page_id = isset($wp_dp_plugin_options['wp_dp_member_dashboard']) ? $wp_dp_plugin_options['wp_dp_member_dashboard'] : $_POST['redirect_to'];
        $redirect_to = wp_dp_wpml_lang_page_permalink( $wp_dp_page_id, 'page' );
        $redirect_to = apply_filters('social_login_redirect_to', $redirect_to);
        $social_login_provider = $_REQUEST['social_login_provider'];
        $wp_dp_provider_identity_key = 'social_login_' . $social_login_provider . '_id';
        $wp_dp_provided_signature = $_REQUEST['social_login_signature'];
        $wp_dp_email = '';
        switch ( $social_login_provider ) {
            case 'facebook':
                ...
                wp_dp_social_login_verify_signature($_REQUEST['social_login_access_token'], $wp_dp_provided_signature, $redirect_to);
				
                $fb_json = json_decode(wp_dp_http_get_contents("https://graph.facebook.com/me?access_token=" . $_REQUEST['social_login_access_token'] . "&fields=" . implode(',', $fields)));
				$facebook_member_profile_url = $fb_json->picture->data->url;
				if ( isset($fb_json->error->type) ? $fb_json->error->type : '' == 'OAuthException' ) {
                    ?>
                    <script>
                        alert("<?php echo wp_dp_plugin_text_srt('wp_dp_social_login_check_fb_account'); ?>");
                        window.close();
                    </script>
                    <?php
                    exit();
                } else {
                    $wp_dp_provider_identity = $fb_json->{ 'id' };
                    $wp_dp_profile_pic = 'https://graph.facebook.com/' . $wp_dp_provider_identity . '/picture';
                    $wp_dp_facebook = $fb_json->{ 'link' };
                    $wp_dp_gender = $fb_json->{ 'gender' };
                    //$wp_dp_email = $fb_json->{ 'email' };
                    if ( isset($fb_json->email) ) {
                        $wp_dp_email = $fb_json->{ 'email' };
                    }
                    $wp_dp_first_name = $fb_json->{ 'first_name' };
                    $wp_dp_last_name = $fb_json->{ 'last_name' };
                    $wp_dp_profile_url = $fb_json->{ 'link' };
                    $wp_dp_gender = $fb_json->{ 'gender' };
                    $wp_dp_name = $wp_dp_first_name . ' ' . $wp_dp_last_name;
                    $user_login = strtolower($wp_dp_first_name . $wp_dp_last_name);
                }
                break;
            case 'twitter':
                $wp_dp_provider_identity = $_REQUEST['social_login_twitter_identity'];
                wp_dp_social_login_verify_signature($wp_dp_provider_identity, $wp_dp_provided_signature, $redirect_to);
                $wp_dp_name = $_REQUEST['social_login_name'];
                $wp_dp_twitter = 'https://twitter.com/' . $_REQUEST['social_login_screen_name'];
                $names = explode(" ", $wp_dp_name);
                $wp_dp_first_name = '';
                if ( isset($names[0]) )
                    $wp_dp_first_name = $names[0];
                $wp_dp_last_name = '';
                if ( isset($names[1]) )
                    $wp_dp_last_name = $names[1];
                $wp_dp_screen_name = $_REQUEST['social_login_screen_name'];
                $facebook_member_profile_url = $_REQUEST['social_profile_image_url'];
                $wp_dp_profile_url = '';
                $wp_dp_gender = '';
                // Get host name from URL
                $site_url = parse_url(site_url());
                $user_login = $wp_dp_screen_name;
                break;
            default:
        ...
        // Get user by meta
        $user_id = wp_dp_social_get_user_by_meta($wp_dp_provider_identity_key, $wp_dp_provider_identity);
        if ( $user_id ) {
            ...
            if ( ($user_roles != '' && in_array("wp_dp_member", $user_roles) ) ) {
                ...
                // update user meta
                update_user_meta($user_id, 'wp_dp_user_last_activity_date', strtotime(date('d-m-Y H:i:s')));
                //update_user_meta($user_id, 'wp_dp_allow_search', 'yes');
                //update_user_meta($user_id, 'wp_dp_user_status', 'active');
                if ( isset($wp_dp_facebook) && $wp_dp_facebook != '' ) {
                    update_user_meta($user_id, 'wp_dp_facebook', $wp_dp_facebook);
                }
                if ( isset($wp_dp_twitter) && $wp_dp_twitter != '' ) {
                    update_user_meta($user_id, 'wp_dp_twitter', $wp_dp_twitter);
                }
            } else {
                ?>
                <script>
                    alert("<?php echo wp_dp_plugin_text_srt('wp_dp_social_login_profile_already_linked'); ?>");
        ...
        } elseif ( $user_id = email_exists($wp_dp_email) ) { // User not found by provider identity, check by email
            $current_user = get_userdata($user_id);
            $user_roles = isset($current_user->roles) ? $current_user->roles : '';
            if ( ($user_roles != '' && in_array("wp_dp_member", $user_roles) ) ) {
                // update user meta
                update_user_meta($user_id, $wp_dp_provider_identity_key, $wp_dp_provider_identity);
                $user_data = get_userdata($user_id);
                $user_login = $user_data->user_login;
                // update user meta
                update_user_meta($user_id, 'wp_dp_user_last_activity_date', strtotime(date('d-m-Y H:i:s')));
            ...
            if ( get_option('users_can_register') ) {
                $member_user_tab = wp_dp_get_transient_obj('member_user_tab');
                if ( empty($wp_dp_email) || $member_user_tab == 'login' ) {
					$data = array(
                        'user_login' => $user_login,
                        'user_email' => $wp_dp_email,
                        'role' => 'wp_dp_member',
                        'first_name' => $wp_dp_first_name,
                        'last_name' => $wp_dp_last_name,
                        'user_url' => $wp_dp_profile_url,
                        'social_login_provider' => $social_login_provider,
                        'social_meta_key' => $wp_dp_provider_identity_key,
                        'social_meta_value' => $wp_dp_provider_identity,
                    );
                ...
                $wp_dp_member_image_id = upload_member_profile_image($facebook_member_profile_url);

                $user_login = wp_dp_get_unique_username($user_login);
                $userdata = array( 'user_login' => $user_login, 'user_email' => $wp_dp_email, 'role' => 'wp_dp_member', 'first_name' => $wp_dp_first_name, 'last_name' => $wp_dp_last_name, 'user_url' => $wp_dp_profile_url, 'user_pass' => wp_generate_password() );
                // Create a new user
                ...
                //do_action('wp_dp_member_register', $reg_user, $random_password);
                do_action('wp_dp_user_register', $reg_user, $random_password);

                $new_user = new WP_User($user_id);
                // update user meta
                $new_user->set_role('wp_dp_member');
                update_user_meta($user_id, 'wp_dp_user_last_activity_date', strtotime(date('d-m-Y H:i:s')));
                update_user_meta($user_id, 'wp_dp_allow_search', 'yes');
                update_user_meta($user_id, 'wp_dp_user_status', 'active');
                ...
                $company_name = $wp_dp_first_name . ' ' . $wp_dp_last_name;
                $company_data = array(
                ...
                if ( $company_ID ) {
                    update_user_meta($user_id, 'wp_dp_user_type', 'supper-admin');
                    update_post_meta($company_ID, 'wp_dp_member_user_type', 'reseller');
                    
                    if ( isset($wp_dp_member_image_id) && $wp_dp_member_image_id != '' ) {
                        update_post_meta($company_ID, 'wp_dp_profile_image', $wp_dp_member_image_id);
                    }
                }

                update_user_meta($user_id, 'show_admin_bar_front', false);
                update_user_meta($user_id, 'wp_dp_is_admin', 1);
                update_user_meta($user_id, 'wp_dp_company', $company_ID);
                ...
                update_user_meta($user_id, 'wp_dp_user_registered', $social_login_provider);
                ...
                if ( isset($wp_dp_plugin_options['wp_dp_member_review_option']) && $wp_dp_plugin_options['wp_dp_member_review_option'] == 'on' ) {
					$member_old_status = get_post_meta($company_ID, 'wp_dp_user_status', true);
                    update_post_meta($company_ID, 'wp_dp_user_status', 'active');
					do_action('wp_dp_profile_status_changed', $company_ID, $member_old_status);
                } else {
					$member_old_status = get_post_meta($company_ID, 'wp_dp_user_status', true);
 ...
 *  login error
 */
if ( ! function_exists('wp_dp_login_errors') ) {

    function wp_dp_login_errors($errors) {
        $errors->errors = array();
        $errors->add('registration_disabled', '<strong>' . wp_dp_plugin_text_srt('wp_dp_social_login_error') . '</strong>:', wp_dp_plugin_text_srt('wp_dp_social_login_reg_disable'));
        return $errors;
    }
 ...
 *  get unique username
 */
if ( ! function_exists('wp_dp_get_unique_username') ) {

    function wp_dp_get_unique_username($user_login, $c = 1) {
        if ( username_exists($user_login) ) {
            ...
            return wp_dp_get_unique_username($user_login, ++ $c);
        } else {
            return $user_login;
        }
    }

    add_action('login_form_social_login', 'wp_dp_social_process_login');
}

/*
 *  AJAX login
 */
if ( ! function_exists('wp_dp_ajax_login') ) {

    function wp_dp_ajax_login() {
        if ( isset($_POST['login_submit']) && $_POST['login_submit'] == 'ajax' && // Plugins will need to pass this param
            ...
            wp_dp_social_process_login(true);
    }

    add_action('init', 'wp_dp_ajax_login');
}
/*
 *  filter user avatar
 */
if ( ! function_exists('wp_dp_filter_avatar') ) {

    function wp_dp_filter_avatar($avatar, $id_or_email, $size, $default, $alt) {
        $custom_avatar = '';
        $social_id = '';
        $provider_id = '';
 ...
 */
if ( ! function_exists('wp_dp_social_add_comment_meta') ) {

    function wp_dp_social_add_comment_meta($comment_id) {
    ...
    add_action('comment_post', 'wp_dp_social_add_comment_meta');
}
/*
 *  social comment meta
 */
if ( ! function_exists('wp_dp_social_comment_meta') ) {

    function wp_dp_social_comment_meta($link) {
        global $comment;
    ...
    add_action('get_comment_author_link', 'wp_dp_social_comment_meta');
}
/*
 *  social login form
 */
if ( ! function_exists('wp_dp_comment_form_social_login') ) {

    function wp_dp_comment_form_social_login() {
        if ( comments_open() && ! is_user_logged_in() ) {
            wp_dp_social_login_form();
        }
    }
 ...
 *  login page url
 */
if ( ! function_exists('wp_dp_login_page_uri') ) {

    function wp_dp_login_page_uri() {
    ...
    add_action('wp_footer', 'wp_dp_login_page_uri');
}
/*
 *  get user by meta key
 */
if ( ! function_exists('wp_dp_social_get_user_by_meta') ) {

    function wp_dp_social_get_user_by_meta($meta_key, $meta_value) {
        global $wpdb;
 ...
 */
if ( ! function_exists('wp_dp_social_generate_signature') ) {

    function wp_dp_social_generate_signature($data) {
 ...
 */
if ( ! function_exists('wp_dp_social_login_verify_signature') ) {

    function wp_dp_social_login_verify_signature($data, $signature, $redirect_to) {
        $generated_signature = wp_dp_social_generate_signature($data);
 ...
 */
if ( ! function_exists('wp_dp_http_get_contents') ) {

    function wp_dp_http_get_contents($url) {
        ...
        if ( is_wp_error($response) ) {
            die(sprintf(wp_dp_plugin_text_srt('wp_dp_social_login_smthng_went_wrong'), $response->get_error_message()));
        } else {
 ...
 *  add custom styling
 */
if ( ! function_exists('wp_dp_add_stylesheets') ) {

    function wp_dp_add_stylesheets() {
        if ( is_admin() ) {
    ...
    add_action('login_enqueue_scripts', 'wp_dp_add_stylesheets');
    add_action('wp_head', 'wp_dp_add_stylesheets');
}

/*
 *  add admin side styling
 */
if ( ! function_exists('wp_dp_add_admin_stylesheets') ) {

    function wp_dp_add_admin_stylesheets() {
        if ( is_admin() ) {
            if ( ! wp_style_is('social_login', 'registered') ) {
                wp_register_style("social_login_css", plugins_url('media/css/cs-social-style.css', __FILE__));
            }
    ...
    add_action('admin_print_styles', 'wp_dp_add_admin_stylesheets');
}
/*
 *  add javascripts files
 */
if ( ! function_exists('wp_dp_add_javascripts') ) {

    function wp_dp_add_javascripts() {
        if ( is_admin() ) {
    ...
    add_action('login_enqueue_scripts', 'wp_dp_add_javascripts');
    add_action('wp_enqueue_scripts', 'wp_dp_add_javascripts');
}
/*
 *  Twitter Callback
 */
if ( ! function_exists('wp_dp_twitter_callback') ) {

    function wp_dp_twitter_callback() {
        ...
        if ( 200 == $connection->http_code ) {
            ...
            $twitter_id = $user->id;
            $signature = wp_dp_social_generate_signature($twitter_id);
            ?>
            <html>
                <head>
                    <script>
            ...
            echo wp_dp_plugin_text_srt('wp_dp_social_login_login_error');
        }
    }
 ...
 *  Twitter connect
 */
if ( ! function_exists('wp_dp_twitter_connect') ) {

    function wp_dp_twitter_connect() {
        global $wp_dp_plugin_options;
        if ( ! class_exists('TwitterOAuth') ) {
            require_once wp_dp::plugin_dir() . 'include/cs-twitter/twitteroauth.php';
        }
        $consumer_key = $wp_dp_plugin_options['wp_dp_consumer_key'];
        $consumer_secret = $wp_dp_plugin_options['wp_dp_consumer_secret'];
        ...
        if ( $consumer_key != '' && $consumer_secret != '' ) {
            ...
            switch ( $connection->http_code ) {
                ...
                default:
                    ...
                    echo wp_dp_plugin_text_srt('wp_dp_social_login_contact_site_admin');
            }
            exit();
        }
    }
 ...
 *  Facebook Callback
 */
if ( ! function_exists('wp_dp_facebook_callback') ) {

    function wp_dp_facebook_callback() {
        global $wp_dp_plugin_options;
        ...
        if ( isset($_GET['code']) ) {
            $code = $_GET['code'];
            $access_token = $code;
            parse_str(wp_dp_http_get_contents("https://graph.facebook.com/oauth/access_token?" .
