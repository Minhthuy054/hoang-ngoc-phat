...
if ( ! class_exists( 'price_tables_post_type_meta' ) ) {
    ...
    class price_tables_post_type_meta {
        ...
        public function __construct() {
            ...
            add_action( 'save_post', array( $this, 'wp_dp_insert_price_table_metas' ), 19 );
            add_action( 'wp_ajax_wp_dp_pt_iconpicker', array( $this, 'wp_dp_pt_icon' ) );

            add_action( 'admin_menu', array( $this, 'remove_cus_meta_boxes' ) );
            add_action( 'do_meta_boxes', array( $this, 'remove_cus_meta_boxes' ) );
            add_filter( "get_user_option_screen_layout_wp_dp-pt", array( $this, 'listing_type_screen_layout' ) );
        }
        ...
        function remove_cus_meta_boxes() {
            remove_meta_box( 'submitdiv', 'wp-dp-pt', 'side' );
        }
        ...
        public function price_tables_add_meta_boxes_callback() {
            add_meta_box( 'wp_dp_meta_price_tables', esc_html( wp_dp_plugin_text_srt( 'wp_dp_listing_price_tables_options' ) ), array( $this, 'wp_dp_meta_price_tables' ), 'wp-dp-pt', 'normal', 'high' );
        }

        /**
         * Creating an array for meta fields
         */
        public function wp_dp_meta_price_tables() {
            global $post, $wp_dp_html_fields;

            $rand_numb = rand( 1000000, 9999999 );
            ?>
            <div class="wp-dp-price-table">
                <div class="row">
                    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                        <?php
                        $wp_dp_opt_array = array(
                            'name' => wp_dp_plugin_text_srt( 'wp_dp_price_subscribe' ),
                            'desc' => '',
                            'hint_text' => '',
                            'echo' => true,
                            'field_params' => array(
                                ...
                                'options' => array( 'order-deatail' => wp_dp_plugin_text_srt( 'wp_dp_price_order_detail' ), 'listing' => wp_dp_plugin_text_srt( 'wp_dp_price_add_listing' ) ),
                                'return' => true,
                            ),
                        );
                ...
                <?php
                    ...
                    $wp_dp_opt_array = array(
                        'cust_type' => 'button',
                        'std' => wp_dp_plugin_text_srt('wp_dp_price_table_add_package'),
                        'id' => 'dir-add-column-'. absint( $rand_numb ),
                        'classes' => 'dir-add-column',
                        'extra_atr' => 'data-id = "'.absint( $rand_numb ).'"', 
                        'prefix_on' => false,
                    );
                    ...
                    $wp_dp_opt_array = array(
                        'cust_type' => 'button',
                        'std' => wp_dp_plugin_text_srt('wp_dp_price_table_add_row'),
                        'id' => 'dir-add-row-'. absint( $rand_numb ),
                        'classes' => 'dir-add-row',
                        'extra_atr' => 'data-id = "'.absint( $rand_numb ).'"', 
                        'prefix_on' => false,
                    );
                    ...
                    $wp_dp_opt_array = array(
                        'cust_type' => 'button',
                        'std' => wp_dp_plugin_text_srt('wp_dp_price_table_add_section'),
                        'id' => 'dir-add-section-'. absint( $rand_numb ),
                        'classes' => 'dir-add-section',
                        'extra_atr' => 'data-id = "'.absint( $rand_numb ).'"', 
                        'prefix_on' => false,
                    );
                    $wp_dp_html_fields->wp_dp_form_text_render($wp_dp_opt_array);
                    ...
                    $wp_dp_opt_array = array(
                        'cust_type' => 'button',
                        'std' => wp_dp_plugin_text_srt('wp_dp_price_table_reset_all'),
                        'id' => 'dir-add-reset-'. absint( $rand_numb ),
                        'classes' => 'dir-cols-reset',
                        'extra_atr' => 'data-id = "'.absint( $rand_numb ).'"', 
                        'prefix_on' => false,
                    );
                    $wp_dp_html_fields->wp_dp_form_text_render($wp_dp_opt_array);
                ...
                <table id="wp-dp-price-table-<?php echo absint( $rand_numb ) ?>" data-id="<?php echo absint( $rand_numb ) ?>" cellpadding="5" cellspacing="0" border="1">
                    <tbody>
                        <?php
                        $table_html = '';
			...
			$wp_dp_currency_sign = wp_dp_get_currency_sign();
                        ...
                        $pt_pkg_name = get_post_meta( $post->ID, 'wp_dp_pt_pkg_names', true );
                        $pt_pkg_price = get_post_meta( $post->ID, 'wp_dp_pt_pkg_prices', true );
                        $pt_pkg_color = get_post_meta( $post->ID, 'wp_dp_pt_pkg_color', true );
                        $pt_pkg_desc = get_post_meta( $post->ID, 'wp_dp_pt_pkg_descs', true );
                        $pt_pkg_btn_txt = get_post_meta( $post->ID, 'wp_dp_pt_pkg_btn_txts', true );
                        $pt_pkg_dur = get_post_meta( $post->ID, 'wp_dp_pt_pkg_durs', true );
                        $pt_pkg_feat = get_post_meta( $post->ID, 'wp_dp_pt_pkg_feats', true );
                        $pt_pkg_url = get_post_meta( $post->ID, 'wp_dp_pt_pkg_urls', true );
                        ...
                        if (
                            ...
                            foreach ( $pt_pkg_name as $pkg_name ) {
                                ...
                                $def_pkg_feat_marup .= '<td class="pkg-featured"><select name="pt_pkg_feat[]"><option value="no">' . wp_dp_plugin_text_srt( 'wp_dp_listing_no' ) . '</option><option' . ($def_pkg_feat == 'yes' ? ' selected="selected"' : '') . ' value="yes">' . wp_dp_plugin_text_srt( 'wp_dp_listing_yes' ) . '</option></select></td>' . "\n";
                                $def_pkg_url_marup .= '<td class="pkg-url">' . $this->wp_dp_pkgs( $def_pkg_url ) . '</td>' . "\n";
                                //
                                $pkgs_counter ++;
                            }
                        }
                        ...
                        if ( $def_pkg_name_marup == '' && $def_pkg_price_marup == '' && $def_pkg_desc_marup == '' ) {
                            ...
                            for ( $pri = 0; $pri < 3; $pri ++ ) {
                                ...
                                $def_pkg_btn_txt_marup .= '<td class="pkg-btn-text"><input type="text" name="pt_pkg_btn_txt[]" value="' . wp_dp_plugin_text_srt( 'wp_dp_price_table_buy_now' ) . '"></td>' . "\n";
                              
                                $def_pkg_feat_marup .= '<td class="pkg-featured"><select name="pt_pkg_feat[]"><option value="no">' . wp_dp_plugin_text_srt( 'wp_dp_listing_no' ) . '</option><option value="yes">' . wp_dp_plugin_text_srt( 'wp_dp_listing_yes' ) . '</option></select></td>' . "\n";
                                $def_pkg_url_marup .= '<td class="pkg-url">' . $this->wp_dp_pkgs( '' ) . '</td>' . "\n";
                            }
                        }

                        $default_field = '
						<tr class="default-fields">
							<td class="pkg-name">' . wp_dp_plugin_text_srt( 'wp_dp_price_name' ) . '</td>
							' . $def_pkg_name_marup . '
						</tr>
						<tr class="default-fields">
							<td class="pkg-price">' . wp_dp_plugin_text_srt( 'wp_dp_price_price' ) . ' ('. $wp_dp_currency_sign .')</td>
							' . $def_pkg_price_marup . '
						</tr>
                        <tr class="default-fields">
							<td class="pkg-color">' . wp_dp_plugin_text_srt( 'wp_dp_price_color' ) . '</td>
							' . $def_pkg_color_marup . '
						</tr>
						<tr class="default-fields">
							<td class="pkg-desc">' . wp_dp_plugin_text_srt( 'wp_dp_price_description' ) . '</td>
							' . $def_pkg_desc_marup . '
						</tr>
						<tr class="default-fields">
							<td class="pkg-btn-text">' . wp_dp_plugin_text_srt( 'wp_dp_price_button_text' ) . '</td>
							' . $def_pkg_btn_txt_marup . '
						</tr>
						
						<tr class="default-fields">
							<td class="pkg-featured">' . wp_dp_plugin_text_srt( 'wp_dp_price_featured' ) . '</td>
							' . $def_pkg_feat_marup . '
						</tr>
						<tr class="default-fields">
							<td class="pkg-url">' . wp_dp_plugin_text_srt( 'wp_dp_price_package' ) . '</td>
							' . $def_pkg_url_marup . '
						</tr>';
                        ...
                        $row_num_input = get_post_meta( $post->ID, 'wp_dp_pt_row_num', true );
                        $pt_col_input = get_post_meta( $post->ID, 'wp_dp_pt_col_vals', true );
                        $pt_col_sub_input = get_post_meta( $post->ID, 'wp_dp_pt_col_subs', true );
                        $pt_row_title = get_post_meta( $post->ID, 'wp_dp_pt_row_title', true );
                        $pt_sec_val = get_post_meta( $post->ID, 'wp_dp_pt_sec_vals', true );
                        $pt_sec_pos = get_post_meta( $post->ID, 'wp_dp_pt_sec_pos', true );
                        if (
                                $row_num_input &&
                            ...
                            if ( $row_break > 0 ) {
                                ...
                                foreach ( $pt_col_input as $col_val ) {
                                    ...
                                    $col_markup .= '<td><input type="text" name="dir_pt_col_val[]" size="10" value="' . $col_val . '"> <br> ' . $this->wp_dp_pt_icon( $pt_sub_input_val, $rand_numb, 'dir_pt_col_sub' ) . (1 == $pt_counter ? $row_num_field : '') . '</td>' . "\n";
                                    $pt_row_del = '';
                            ...
                            if ( $price_tables_blank ) {
                                ...
                                for ( $pri = 0; $pri < 6; $pri ++ ) {
                                    $rand_numb = rand( 10000000, 99999999 );
                                    $col_markup .= '<td><input type="text" name="dir_pt_col_val[]" size="10" value="Title"> <br> ' . $this->wp_dp_pt_icon( '', $rand_numb, 'dir_pt_col_sub' ) . (1 == $pt_counter ? $row_num_field : '') . '</td>' . "\n";
                                    $pt_row_del = '';
                                    if ( $row_break == $pt_counter ) {
            ...
            <?php
            $this->submit_meta_box( 'wp-dp-pt', $args = array() );
        }
        ...
        public function wp_dp_pkgs( $value = '' ) {
            ...
            $pkgs_options .= '<option value="">' . wp_dp_plugin_text_srt( 'wp_dp_price_select_package' ) . '</option>';
        ...
        public function wp_dp_insert_price_table_metas( $post_id ) {

            if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
                return;
            }

            $row_num_input = wp_dp_get_input( 'dir_pt_row_num', '', 'ARRAY' );
            $pt_col_input = wp_dp_get_input( 'dir_pt_col_val', '', 'ARRAY' );
            $pt_col_sub_input = wp_dp_get_input( 'dir_pt_col_sub', '', 'ARRAY' );
            $pt_row_title = wp_dp_get_input( 'dir_pt_row_title', '', 'ARRAY' );
            $pt_sec_val = wp_dp_get_input( 'dir_pt_sec_val', '', 'ARRAY' );
            $pt_sec_pos = wp_dp_get_input( 'dir_pt_sec_pos', '', 'ARRAY' );

            $pt_pkg_name = wp_dp_get_input( 'pt_pkg_name', '', 'ARRAY' );
            $pt_pkg_price = wp_dp_get_input( 'pt_pkg_price', '', 'ARRAY' );
            $pt_pkg_color = wp_dp_get_input( 'pt_pkg_color', '', 'ARRAY' );
            $pt_pkg_desc = wp_dp_get_input( 'pt_pkg_desc', '', 'ARRAY' );
            $pt_pkg_btn_txt = wp_dp_get_input( 'pt_pkg_btn_txt', '', 'ARRAY' );
            $pt_pkg_dur = wp_dp_get_input( 'pt_pkg_dur', '', 'ARRAY' );
            $pt_pkg_feat = wp_dp_get_input( 'pt_pkg_feat', '', 'ARRAY' );
            $pt_pkg_url = wp_dp_get_input( 'pt_pkg_url', '', 'ARRAY' );
            
            
            if(isset($row_num_input) && !empty($row_num_input) ){
               $row_num_input = absint( sizeof( $row_num_input ) ); 
            }
            
            
            // saving package names		
            update_post_meta( $post_id, 'wp_dp_pt_pkg_names', $pt_pkg_name );

            // saving package prices		
            update_post_meta( $post_id, 'wp_dp_pt_pkg_prices', $pt_pkg_price );
            update_post_meta( $post_id, 'wp_dp_pt_pkg_color', $pt_pkg_color );

            // saving package descs		
            update_post_meta( $post_id, 'wp_dp_pt_pkg_descs', $pt_pkg_desc );

            // saving package btn texts		
            update_post_meta( $post_id, 'wp_dp_pt_pkg_btn_txts', $pt_pkg_btn_txt );

            // saving package durs		
            update_post_meta( $post_id, 'wp_dp_pt_pkg_durs', $pt_pkg_dur );

            // saving package feats		
            update_post_meta( $post_id, 'wp_dp_pt_pkg_feats', $pt_pkg_feat );

            // saving package urls		
            update_post_meta( $post_id, 'wp_dp_pt_pkg_urls', $pt_pkg_url );

            // saving num of rows			
            update_post_meta( $post_id, 'wp_dp_pt_row_num',$row_num_input  );

            // saving all cols values			
            update_post_meta( $post_id, 'wp_dp_pt_col_vals', $pt_col_input );

            // saving all cols sub values			
            update_post_meta( $post_id, 'wp_dp_pt_col_subs', $pt_col_sub_input );

            // saving row titles values			
            update_post_meta( $post_id, 'wp_dp_pt_row_title', $pt_row_title );

            // saving all section values
            update_post_meta( $post_id, 'wp_dp_pt_sec_vals', $pt_sec_val );

            // saving all section pos values
            update_post_meta( $post_id, 'wp_dp_pt_sec_pos', $pt_sec_pos );
        }

        public function wp_dp_pt_icon( $value = '', $id = '', $name = '' ) {//begin function
            if ( $value == '' && $id == '' && $name == '' ) {
			...
			<span id="e9_buttons_' . $id . '" style="display:none">\
				<button autocomplete="off" type="button" class="btn btn-primary">'.wp_dp_plugin_text_srt('wp_dp_load_icomoon_selection_json').'</button>
			</span>';
