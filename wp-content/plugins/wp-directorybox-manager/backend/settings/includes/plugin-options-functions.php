...
if ( ! function_exists('plugin_option_save') ) {
    ...
    function plugin_option_save() {
        global $reset_plugin_data, $wp_dp_setting_options;
        $_POST  = apply_filters( 'wp_dp_plugin_options_save', $_POST );
        $_POST['wp_dp_linkedin_app_redirect_uri'] = site_url();
        $_POST = stripslashes_htmlspecialchars($_POST); 
        update_option("wp_dp_plugin_options", $_POST);
        wp_dp_update_extras_options();
        wp_dp_update_packages_options();
        wp_dp_update_feats();
        $response = wp_dp_plugin_text_srt('wp_dp_opt_func_option_saved');

        echo wp_dp_cs_allow_special_char($response);
        die();
    }
...
if ( ! function_exists('wp_dp_check_plugin_fields_avail') ) {

    function wp_dp_check_plugin_fields_avail() {
        ...
        if ( $name == '' ) {
            ...
            $wp_dp_json['message'] = esc_html('Required field', 'wp-dp');
        } elseif ( preg_match('/\s/', $name) ) {
            $error_flag = true;
            $wp_dp_json['type'] = 'error';
            $wp_dp_json['message'] = esc_html('Space not Allowed', 'wp-dp');
        } elseif ( preg_match('/[\'^�$%&*()}{@#~?><>,|=+�]/', $name) ) {
            $error_flag = true;
            $wp_dp_json['type'] = 'error';
            $wp_dp_json['message'] = esc_html('Special characters are not Allowed', 'wp-dp');
        } elseif ( is_array($member_value) && sizeof($member_value) > 0 ) {
            foreach ( $member_value as $key => $value ) {
                if ( $name == $value ) {
                    $error_flag = true;
                    $wp_dp_json['type'] = 'error';
                    $wp_dp_json['message'] = esc_html('Name already exists', 'wp-dp');
                }
            }
        }
        if ( $error_flag != true ) {
            $wp_dp_json['type'] = 'success';
            $wp_dp_json['message'] = esc_html('Name Available', 'wp-dp');
        }
        echo json_encode($wp_dp_json);
        wp_die();
    }

    add_action('wp_ajax_wp_dp_check_plugin_fields_avail', 'wp_dp_check_plugin_fields_avail');
}
 ...
 */
if ( ! function_exists('wp_dp_pl_opt_backup_generate') ) {

    function wp_dp_pl_opt_backup_generate() {
        global $wp_filesystem;
		
		require_once ABSPATH . '/wp-admin/includes/file.php';
		
        $wp_dp_export_options = get_option('wp_dp_plugin_options');
        $wp_dp_export_options = apply_filters('wp_dp_translate_options', $wp_dp_export_options );
        $wp_dp_option_fields = json_encode($wp_dp_export_options, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE);
        ...
        $wp_dp_filename = trailingslashit($wp_dp_upload_dir) . (current_time('d-M-Y_H.i.s', 1)) . '.json';


        if ( ! $wp_filesystem->put_contents($wp_dp_filename, $wp_dp_option_fields, FS_CHMOD_FILE) ) {
            echo wp_dp_plugin_text_srt('wp_dp_opt_func_error_saving');
        } else {
            echo wp_dp_plugin_text_srt('wp_dp_opt_func_backup_generated');
        }
        die();
    }

    add_action('wp_ajax_wp_dp_pl_opt_backup_generate', 'wp_dp_pl_opt_backup_generate');
}
/**
 ...
 */
if ( ! function_exists('wp_dp_get_settings_demo') ) {

    function wp_dp_get_settings_demo($wp_dp_demo_file = '') {
        global $wp_filesystem;
        ...
        $wp_dp_upload_dir = wp_dp::plugin_dir() . 'backend/settings/demo/';
        $wp_dp_filename = trailingslashit($wp_dp_upload_dir) . $wp_dp_demo_file;
        $wp_dp_demo_data = array();
        if ( is_file($wp_dp_filename) ) {
            $get_options_file = $wp_filesystem->get_contents($wp_dp_filename);

            $wp_dp_demo_data = $get_options_file;
        }
        return $wp_dp_demo_data;
    }
...
if ( ! function_exists('wp_dp_demo_plugin_data') ) {
    ...
    function wp_dp_demo_plugin_data($filename = "", $obj = array()) {
        ...
        if ( $filename == "" ) {
            ...
            $demo_plugin_data = '';
            if ( isset($wp_dp_settings_init) && $wp_dp_settings_init <> '' ) {
                $wp_dp_settings = $wp_dp_settings_init['plugin_options'];
                $plugin_settings = json_decode($wp_dp_settings, true);
                $demo_plugin_data = $plugin_settings;
            }
        } else {
            global $wp_filesystem;
            $wp_dp_settings = $wp_filesystem->get_contents($filename);
            if ( isset($obj) && ! empty($obj) ) {
                ...
                $wp_dp_settings = str_replace(str_replace('"', '', json_encode($obj->attachments_replace_url)), str_replace('"', '', json_encode($obj->wp_upload_url_path)), $wp_dp_settings);
                //
            }
            $plugin_settings = json_decode($wp_dp_settings, true);
            $demo_plugin_data = $plugin_settings;
        ...
        delete_option('wp_dp_plugin_options');
        update_option("wp_dp_plugin_options", $demo_plugin_data);
    }
 ...
 */
if ( ! function_exists('wp_dp_pl_backup_file_delete') ) {

    function wp_dp_pl_backup_file_delete() {
        global $wp_filesystem;
        ...
        if ( is_file($wp_dp_filename) ) {
            ...
            printf(wp_dp_plugin_text_srt('wp_dp_opt_func_file_delete'), $file_name);
        } else {
            echo wp_dp_plugin_text_srt('wp_dp_opt_func_error_delete');
        }
        die();
    }

    add_action('wp_ajax_wp_dp_pl_backup_file_delete', 'wp_dp_pl_backup_file_delete');
}
/**
 ...
 */
if ( ! function_exists('wp_dp_pl_backup_file_restore') ) {

    function wp_dp_pl_backup_file_restore() {
        global $wp_filesystem;
        ...
        if ( $file_path == 'yes' ) {
            ...
            if ( $wp_dp_file_body != '' ) {
                $get_options_file = json_decode($wp_dp_file_body, true);
                update_option("wp_dp_plugin_options", $get_options_file);

                echo wp_dp_plugin_text_srt('wp_dp_options_file_import_success');
            } else {
                echo wp_dp_plugin_text_srt('wp_dp_options_error_restoring_file');
            }
            die;
        }
        $wp_dp_filename = trailingslashit($wp_dp_upload_dir) . $file_name;
        if ( is_file($wp_dp_filename) ) {
            ...
            update_option("wp_dp_plugin_options", $get_options_file);
            printf(wp_dp_plugin_text_srt('wp_dp_opt_func_restore_file'), $file_name);
        } else {
            echo wp_dp_plugin_text_srt('wp_dp_options_error_restoring_file');
        }
        die();
    ...
    add_action('wp_ajax_wp_dp_pl_backup_file_restore', 'wp_dp_pl_backup_file_restore');
}
/**
...
if ( ! function_exists('plugin_option_rest_all') ) {
    ...
    function plugin_option_rest_all() {
        ...
        $wp_dp_upload_dir = wp_dp::plugin_dir() . 'backend/settings/default-settings/';
        $wp_dp_filename = trailingslashit($wp_dp_upload_dir) . 'default-settings.json';
        if ( is_file($wp_dp_filename) ) {
            $get_options_file = $wp_filesystem->get_contents($wp_dp_filename);
            $get_options_file = json_decode($get_options_file, true);
            update_option("wp_dp_plugin_options", $get_options_file);
 ...
 */
if ( ! function_exists('wp_dp_update_packages_options') ) {

    function wp_dp_update_packages_options() {
        $data = get_option("wp_dp_plugin_options");
        $package_counter = 0;
        ...
        if ( isset($_POST['package_id_array']) && ! empty($_POST['package_id_array']) ) {
            foreach ( $_POST['package_id_array'] as $keys => $values ) {
                if ( $values ) {
                    ...
                    $package_array['wp_dp_list_dur'] = $_POST['wp_dp_list_dur_array'][$package_counter];
        ...
        // Update Packages
        $packagesdata['wp_dp_packages_options'] = $packages;
        $wp_dp_options = array_merge($data, $packagesdata);
 ...
 * Update Extras
 * ----------------------------------------------------- */
if ( ! function_exists('wp_dp_update_extras_options') ) {

    function wp_dp_update_extras_options() {
        $data = get_option("wp_dp_plugin_options");
        ...
        if ( isset($_POST['extra_feature_id_array']) && ! empty($_POST['extra_feature_id_array']) ) {
            foreach ( $_POST['extra_feature_id_array'] as $keys => $values ) {
                if ( $values ) {
                    ...
                    $extra_feature_array['wp_dp_extra_feature_title'] = $_POST['wp_dp_extra_feature_title_array'][$extra_feature_counter];
                    $extra_feature_array['wp_dp_extra_feature_price'] = $_POST['wp_dp_extra_feature_price_array'][$extra_feature_counter];
                    $extra_feature_array['wp_dp_extra_feature_type'] = $_POST['wp_dp_extra_feature_type_array'][$extra_feature_counter];
                    $extra_feature_array['wp_dp_extra_feature_guests'] = $_POST['wp_dp_extra_feature_guests_array'][$extra_feature_counter];
                    $extra_feature_array['wp_dp_extra_feature_fchange'] = $_POST['wp_dp_extra_feature_fchange_array'][$extra_feature_counter];
                    $extra_feature_array['wp_dp_extra_feature_desc'] = $_POST['wp_dp_extra_feature_desc_array'][$extra_feature_counter];
        ...
        $wp_dp_options = array_merge($data, $extrasdata);
        update_option("wp_dp_plugin_options", $wp_dp_options);
        $obj = new wp_dp_plugin_options();
        $obj->wp_dp_remove_duplicate_extra_value();
    }
 ...
 */
if ( ! function_exists('wp_dp_update_feats') ) {

    function wp_dp_update_feats() {
        $data = get_option("wp_dp_plugin_options");
        $feats_counter = 0;
        ...
        if ( isset($_POST['feats_id_array']) && ! empty($_POST['feats_id_array']) ) {
            foreach ( $_POST['feats_id_array'] as $keys => $values ) {
                if ( $values ) {
                    ...
                    $feats_array['wp_dp_feats_title'] = $_POST['wp_dp_feats_title_array'][$feats_counter];
                    $feats_array['wp_dp_feats_image'] = $_POST['wp_dp_feats_image_array'][$feats_counter];
                    $feats_array['wp_dp_feats_desc'] = $_POST['wp_dp_feats_desc_array'][$feats_counter];
                    $feats[$values] = $feats_array;
                    $feats_counter ++;
                }
            }
        }
        $extrasdata['wp_dp_feats_options'] = $feats;
        $wp_dp_options = array_merge($data, $extrasdata);
        update_option("wp_dp_plugin_options", $wp_dp_options);
    }
 ...
 */
if ( ! function_exists('wp_dp_get_currency_symbol') ) {

    function wp_dp_get_currency_symbol() {
        $code = $_POST['code'];
        $currency_list = wp_dp_get_currency();
        echo WP_DP_FUNCTIONS()->special_chars($currency_list[$code]['symbol']);
        die();
    }

    add_action('wp_ajax_wp_dp_get_currency_symbol', 'wp_dp_get_currency_symbol');
}
/**
 * end Function  how to get currency Symbols
 */
if ( ! function_exists("wp_dp_get_lk_page") ) {

    function wp_dp_get_lk_page($url, $ref = '', $ctOnly = false, $fields = '', $advSettings = '', $ch = false) {
        ...
        if ( $http_code == 301 || $http_code == 302 || $http_code == 303 ) {
            ...
            return wp_dp_get_lk_page($new_url, $last_urlX, $ctOnly, '', $advSettings, $ch);
        } else {
            $curl_loops = 0;
            curl_close($ch);
            return ($ctOnly === true) ? $content : $fullresponse;
...
if ( ! function_exists('wp_dp_get_yelp_access_token_callback') ) {

    function wp_dp_get_yelp_access_token_callback() {
        ...
        $token['token'] = isset($yelp_data['access_token']) ? $yelp_data['access_token'] : '';
//        $token['client_id'] = isset($yelp_client_id) ? $yelp_client_id : '';
//        $token['secret_id'] = isset($yelp_secret_id) ? $yelp_secret_id : '';
        echo json_encode($token );
        
        die;
    }

    add_action('wp_ajax_wp_dp_get_yelp_access_token', 'wp_dp_get_yelp_access_token_callback');
}
 ...
 *
 */
if ( ! function_exists('wp_dp_banner_ads_banner') ) {

    function wp_dp_banner_ads_banner() {

        global $wp_dp_html_fields, $wp_dp_form_fields, $wp_dp_static_text;
        ...
        $wp_dp_html .='<li class="wp-dp-list-item">
        ...
        if ( $_POST['banner_type_input'] == 'image' ) {
            ...
            $wp_dp_html .= '<img src="' . esc_url($img_url[0]) . '" alt="" width="100" />';
            $wp_dp_html .= '&nbsp;';
        } else {
            $wp_dp_html .= '<td>' . wp_dp_plugin_text_srt('wp_dp_banner_custom_code') . '</td>';
            $wp_dp_html .= '<td>&nbsp;</td>';
        }
        $wp_dp_html .='</div>
                </div>
            ...
            <div class="col-lg-2 col-md-2 col-sm-6 col-xs-12">
                ...
                <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                    ...
                    <div class="input-element">
                        <div class="input-holder">
                            [wp_dp_ads id="' . absint($wp_dp_rand_num) . '"]
                        </div>
                    </div>
                </div>
        ...
        $wp_dp_html .= '<div class="parent-li-edit-div banner-style" id="' . absint($wp_dp_rand_num) . '" style="display:none">';
        $wp_dp_opt_array = array(
            'name' => wp_dp_plugin_text_srt('wp_dp_banner_title_field'),
            'desc' => '',
            'hint_text' => wp_dp_plugin_text_srt('wp_dp_banner_title_field_hint'),
            'field_params' => array(
                'std' => isset($_POST['banner_title_input']) ? $_POST['banner_title_input'] : '',
                'cust_id' => 'banner_title' . absint($wp_dp_rand_num),
        ...
        $wp_dp_opt_array = array(
            ...
            'field_params' => array(
                ...
                'cust_id' => 'banner_style' . absint($wp_dp_rand_num),
        ...
        $wp_dp_opt_array = array(
            ...
            'hint_text' => wp_dp_plugin_text_srt('wp_dp_banner_type_hint'),
            'field_params' => array(
                'std' => isset($_POST['banner_type_input']) ? $_POST['banner_type_input'] : '',
                'cust_id' => 'banner_type' . absint($wp_dp_rand_num),
                ...
                array(
                    ...
                    'code' => wp_dp_plugin_text_srt('wp_dp_banner_code'),
                ),
                'return' => true,
            ),
        );
        ...
        $wp_dp_html .='<div class="form-elements" id="ads_image' . absint($wp_dp_rand_num) . '" style="display:' . esc_html($display_ads) . '">';
        $wp_dp_opt_array = array(
            ...
            'hint_text' => wp_dp_plugin_text_srt('wp_dp_banner_image_hint'),
            'prefix' => '',
            'array' => true,
            'field_params' => array(
                'std' => isset($_POST['image_path']) ? $_POST['image_path'] : '',
        ...
        $wp_dp_html .= $wp_dp_html_fields->wp_dp_upload_file_field($wp_dp_opt_array);
        $wp_dp_html .='</div>';

        $wp_dp_opt_array = array(
            'name' => wp_dp_plugin_text_srt('wp_dp_banner_url_field'),
            'desc' => '',
            'hint_text' => wp_dp_plugin_text_srt('wp_dp_banner_url_hint'),
            'field_params' => array(
                'std' => isset($_POST['banner_field_url_input']) ? $_POST['banner_field_url_input'] : '',
                'cust_id' => 'banner_field_url' . absint($wp_dp_rand_num),
        ...
        $wp_dp_opt_array = array(
            ...
            'hint_text' => wp_dp_plugin_text_srt('wp_dp_banner_target_hint'),
            'field_params' => array(
                'std' => isset($_POST['banner_target_input']) ? $_POST['banner_target_input'] : '',
                'desc' => '',
                'cust_id' => 'banner_target' . absint($wp_dp_rand_num),
        ...
        $wp_dp_html .='<div id="ads_code' . absint($wp_dp_rand_num) . '" style="display:' . esc_html($display_ads) . '">';
        $wp_dp_opt_array = array(
            'name' => wp_dp_plugin_text_srt('wp_dp_banner_ad_sense_code'),
            'desc' => '',
            'hint_text' => wp_dp_plugin_text_srt('wp_dp_banner_ad_sense_code_hint'),
            'field_params' => array(
                'std' => isset($_POST['adsense_code_input']) ? $_POST['adsense_code_input'] : '',
                'cust_id' => 'adsense_code' . absint($wp_dp_rand_num),
        ...
        $wp_dp_opt_array = array(
            'std' => absint($wp_dp_rand_num),
            'id' => 'banner_field_code_no' . absint($wp_dp_rand_num),
    ...
    add_action('wp_ajax_wp_dp_banner_ads_banner', 'wp_dp_banner_ads_banner');
}
 ...
 */
if ( ! function_exists('wp_dp_get_currencies') ) {

    function wp_dp_get_currencies() {
        return array(
...
if ( ! function_exists('wp_dp_load_all_pages_callback') ) {
    add_action('wp_ajax_wp_dp_load_all_pages', 'wp_dp_load_all_pages_callback');

    function wp_dp_load_all_pages_callback() {
        $args = isset($_POST['args']) ? $_POST['args'] : '';
        $new_args = json_decode(stripslashes($args), true);
        $wp_dp_output = wp_dropdown_pages($new_args);
        $wp_dp_output .= '<script type="text/javascript">
			jQuery(document).ready(function () {
				chosen_selectionbox();
			});
		</script>';
        echo json_encode(array( 'html' => $wp_dp_output ));
        die;
    }

}

if ( ! function_exists('wp_dp_load_all_agencies_options_callback') ) {
    add_action('wp_ajax_wp_dp_load_all_agencies_options', 'wp_dp_load_all_agencies_options_callback');

    function wp_dp_load_all_agencies_options_callback() {
        ...
        $wp_dp_users = get_users($args);
        foreach ( $wp_dp_users as $user ) {
            $wp_dp_company_id = get_user_meta($user->ID, 'wp_dp_company', true);
            $wp_dp_agency_list[$user->ID] = $user->display_name;
        }
...
if ( ! function_exists('wp_dp_load_all_members_options_callback') ) {
    add_action('wp_ajax_wp_dp_load_all_members_options', 'wp_dp_load_all_members_options_callback');

    function wp_dp_load_all_members_options_callback() {
        global $wp_dp_plugin_options, $wp_dp_form_fields;
        ...
        foreach ( $wp_dp_users as $user ) {
            $wp_dp_company_id = get_user_meta($user->ID, 'wp_dp_company', true);
            $wp_dp_member_list[$user->ID] = $user->display_name;
        }
