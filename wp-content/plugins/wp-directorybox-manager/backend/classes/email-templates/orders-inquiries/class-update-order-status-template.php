...
if ( ! class_exists( 'Wp_dp_order_status_updated_email_template' ) ) {

	class Wp_dp_order_status_updated_email_template {
		...
		public function __construct() {
			...
			add_action( 'wp_dp_order_status_updated_email', array( $this, 'wp_dp_order_status_updated_email_callback' ), 10, 4 );
		}

		public function wp_dp_order_status_updated_email_callback( $order_id = '' ) {
			...
			if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
				...
				$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : wp_dp_plugin_text_srt( 'wp_dp_update_order_status' );
				...
				Wp_dp_order_status_updated_email_template::$is_email_sent1 = $this->is_email_sent;
			}
		}
		...
		public function add_email_template() {
			...
			$email_templates[$this->template_group][$this->email_template_index] = array(
				...
				'description' => wp_dp_plugin_text_srt( 'wp_dp_update_order_status_email' ),
				'jh_email_type' => 'html',
			);
		...
		function get_order_user_name() {
			$order_user_id   = get_post_meta( $this->order_id, 'wp_dp_order_user', true );
		...
		function get_listing_user_name() {
			$listing_user_id   = get_post_meta( $this->order_id, 'wp_dp_member', true );
		...
		function get_listing_title() {
			$listing_id   = get_post_meta( $this->order_id, 'wp_dp_listing_id', true );
		...
		function get_order_link() {
			...
			if( $member_dashboard != '' ){
				return wp_dp_wpml_lang_page_permalink( $member_dashboard, 'page' ).'?dashboard=orders';
			}else{
				return esc_url( site_url( '/dashboard/?dashboard=orders' ) );
			}
		}
		function get_order_status() {
			$order_status = get_post_meta( $this->order_id, 'wp_dp_order_status', true );
	...
	new Wp_dp_order_status_updated_email_template();
}
