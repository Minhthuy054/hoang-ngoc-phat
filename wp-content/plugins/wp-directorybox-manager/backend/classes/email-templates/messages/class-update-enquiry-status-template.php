 ...
 */
if ( ! class_exists( 'Wp_dp_enquiry_status_updated_email_template' ) ) {

	class Wp_dp_enquiry_status_updated_email_template {
		...
		public function __construct() {
			...
			$this->email_template_type = 'Inquiry Update to User';
			...
			$this->email_template_variables = array(
				...
				array(
					...
					'value_callback' => array( $this, 'get_enquiry_number' ),
				),
				array(
					'tag' => 'INQUIRY_LINK',
					'display_text' => 'Inquiry LINK',
					'value_callback' => array( $this, 'get_enquiry_link' ),
				),
				array(
					'tag' => 'INQUIRY_STATUS',
					'display_text' => 'Inquiry Status',
					'value_callback' => array( $this, 'get_enquiry_status' ),
				),
			);
			$this->template_group = 'Messages';

			$this->email_template_index = 'enquiry-status-updated-template';
			...
			add_action( 'wp_dp_enquiry_status_updated_email', array( $this, 'wp_dp_enquiry_status_updated_email_callback' ), 10, 4 );
		}

		public function wp_dp_enquiry_status_updated_email_callback( $enquiry_id = '' ) {
			...
			if ( isset( $template['email_notification'] ) && $template['email_notification'] == 1 ) {
				...
				$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : wp_dp_plugin_text_srt( 'wp_dp_update_inquiry_email_subject' );
				$from = (isset( $template['from'] ) && $template['from'] != '') ? $template['from'] : esc_attr( $this->get_listing_user_name() ) . ' <' . $this->get_listing_user_email() . '>';
				$recipients = (isset( $template['recipients'] ) && $template['recipients'] != '') ? $template['recipients'] : $this->get_enquiry_user_email();
				...
				Wp_dp_enquiry_status_updated_email_template::$is_email_sent1 = $this->is_email_sent;
			}
		}
		...
		public function add_email_template() {
			...
			$email_templates[$this->template_group][$this->email_template_index] = array(
				...
				'description' => wp_dp_plugin_text_srt( 'wp_dp_update_inquiry_email' ),
				'subject' => wp_dp_plugin_text_srt( 'wp_dp_update_inquiry_email_subject' ),
				'jh_email_type' => 'html',
			);
		...
		function get_enquiry_user_name() {
			$enquiry_member = get_post_meta( $this->enquiry_id, 'wp_dp_enquiry_member', true );
			return esc_html( get_the_title( $enquiry_member ) );
		}

		function get_enquiry_user_email() {
			$enquiry_member = get_post_meta( $this->enquiry_id, 'wp_dp_enquiry_member', true );
			$viewing_member_email_address = get_post_meta( $enquiry_member, 'wp_dp_email_address', true );
			return $viewing_member_email_address;
		}

		function get_listing_user_name() {
			$listing_member = get_post_meta( $this->enquiry_id, 'wp_dp_listing_member', true );
		...
		function get_listing_title() {
			$listing_id = get_post_meta( $this->enquiry_id, 'wp_dp_listing_id', true );
		...
		function get_enquiry_number() {
			return $this->enquiry_id;
		}

		function get_enquiry_link() {
			...
			if ( $member_dashboard != '' ) {
				return wp_dp_wpml_lang_page_permalink( $member_dashboard, 'page' ) . '?dashboard=enquiries';
			} else {
				return esc_url( site_url( '/dashboard/?dashboard=enquiries' ) );
			}
		}

		function get_enquiry_status() {
			$enquiry_status = get_post_meta( $this->enquiry_id, 'wp_dp_enquiry_status', true );
			return esc_html( $enquiry_status );
		}
							...
							<tbody>
								<tr>
									<td style="padding: 40px 30px 30px 30px;" align="center" bgcolor="#33333e">
										<h1 style="color: #fff;">Inquiry Update to User</h1>
									</td>
								</tr>
								<tr>
										...
										<table border="0" cellpadding="0" cellspacing="0" width="100%">
											<tr>
												<td width="260" valign="top">
													<table border="0" cellpadding="0" cellspacing="0" width="100%">
														<tr>
															<td>Dear [INQUIRY_USER_NAME]</td>
														</tr>
														<tr>
															<td style="padding-bottom: 8px;">Thank you for working with [SITE_NAME]. Your inquiry has been updated. Below is the detail of updated inquiry;</td>
														</tr>
														<tr>
															<td style="padding-bottom: 8px;">Listing Title: <a href="[LISTING_LINK]">[LISTING_TITLE]</a></td>
														</tr>
														<tr>
															<td style="padding-bottom: 8px;">Inquiry Status : [INQUIRY_STATUS]</td>
														</tr>
														<tr>
															<td style="padding-bottom: 8px;">You can see inquiry on following link:</td>
														</tr>
														<tr>
															<td style="padding-bottom: 8px;">[INQUIRY_LINK]</td>
														</tr>
														<tr>
	...
	new Wp_dp_enquiry_status_updated_email_template();
}
