...
if ( ! class_exists( 'Wp_dp_listing_not_approved_email_template' ) ) {

	class Wp_dp_listing_not_approved_email_template {
		...
		public function __construct() {
			...
			$this->email_template_type = 'Rejected Listing Notification to Listing Owner';
			...
			$this->email_template_index = 'listing-not-approved-template';

			add_filter( 'wp_dp_email_template_settings', array( $this, 'template_settings_callback' ), 12, 1 );

			// Add action listing status callback
			add_action( 'wp_dp_listing_not_approved_email', array( $this, 'listing_status_changed_callback' ), 10, 2 );
		...
		public function listing_status_changed_callback( $listing_id ) {
			...
			if ( $listing_id != '' ) {
				...
				if ( isset( $template['email_notification'] ) && $template['email_notification'] ) {
					...
					$subject = (isset( $template['subject'] ) && $template['subject'] != '' ) ? $template['subject'] : wp_dp_plugin_text_srt( 'wp_dp_listing_not_approved_email_subject' );
		...
		public function add_email_template() {
			...
			$email_templates[$this->template_group][$this->email_template_index] = array(
				...
				'description' => wp_dp_plugin_text_srt( 'wp_dp_listing_not_approved_email' ),
				'subject' => wp_dp_plugin_text_srt( 'wp_dp_listing_not_approved_email_subject' ),
				'jh_email_type' => 'html',
			);
							...
							<tbody>
								<tr>
									<td style="padding: 40px 30px 30px 30px;" align="center" bgcolor="#33333e">
										<h1 style="color: #fff;">Rejected Listing Notification to Listing Owner</h1>
									</td>
								</tr>
								<tr>
											...
											<tr>
												<td width="260" valign="top">
													<table border="0" cellpadding="0" cellspacing="0" width="100%">
														...
														<tr>
															<td style="padding-bottom: 8px;">Thank you for submitting your listing with [SITE_NAME]. Your listing has not been approved. Please make the required changes and submit it again. We will be glade to approve it after required changes. Your listing details are as following;</td>
														</tr>
														<tr>
	...
	new Wp_dp_listing_not_approved_email_template();
}